...
bool areEqual(const std::any &a, const std::any &b)
{
    ...
    try
    {
        ...
        else if (a.type() == typeid(std::vector<std::any>))
        {
            ...
            return vecA.size() == vecB.size() && std::equal(vecA.begin(), vecA.end(), vecB.begin(), [](const std::any &x, const std::any &y)
                                                            { return areEqual(x, y); });
        }
...
bool inOp(const std::any &obj, const std::any &key)
{
    ...
    try
    {
        ...
        if (obj.type() == typeid(std::vector<std::any>))
        {
            ...
            return std::any_of(vec.begin(), vec.end(), [&key](const std::any &element)
                               { return areEqual(element, key); });
        }
...
int getIndexOf(const std::any &collection, const std::any &target)
{
    ...
    try
    {
        ...
        if (collection.type() == typeid(std::vector<std::any>))
        {
            ...
            auto it = std::find_if(vec.begin(), vec.end(), [&target](const std::any &elem)
                                   { return areEqual(elem, target); });
...
void setValue(std::any &target, const std::any &prop, const std::any &value)
{
    try
    {
        ...
        if (target.type() == typeid(std::unordered_map<std::string, std::any>))
        {
            std::unordered_map<std::string, std::any> dict = std::any_cast<std::unordered_map<std::string, std::any> &>(target);
            if (prop.type() == typeid(std::string))
            {
                const std::string &key = std::any_cast<const std::string &>(prop);
                dict[key] = value;
            }
            else if (prop.type() == typeid(std::any))
            {
                const std::string &key = std::any_cast<const std::string &>(prop);
                dict[key] = value;
            }
            else if (prop.type() == typeid(const char *))
            {
                std::string key = std::string(std::any_cast<const char *>(prop));
                dict[key] = value;
            }
            else
            {
                throw std::invalid_argument("Invalid key type for dictionary");
            }
        }
        // Check if target is a list
        else if (target.type() == typeid(std::vector<std::any>))
        {
        ...
        {
            throw std::invalid_argument("Unsupported target type");
        }
    }
    catch (const std::bad_any_cast &e)
    {
