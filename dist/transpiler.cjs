...
`;
  ...
  goBoxedElementRead(node, printedText) {
    while (_optionalChain([node, 'optionalAccess', _284 => _284.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
      node = node.expression;
    }
    if (_optionalChain([node, 'optionalAccess', _285 => _285.kind]) !== _typescript2.default.SyntaxKind.ElementAccessExpression) {
      return false;
    }
    if (GO_ANY_BOX_CALLS.indexOf(this.goPrintedCallee(printedText)) >= 0) {
      return true;
...
${this.getIden(level)}}()`;
  ...
  printInlineEquality(left, right, leftText, rightText, isEq) {
    ...
    const isBoolLiteral = (node) => _optionalChain([node, 'optionalAccess', _334 => _334.kind]) === _typescript2.default.SyntaxKind.TrueKeyword || _optionalChain([node, 'optionalAccess', _335 => _335.kind]) === _typescript2.default.SyntaxKind.FalseKeyword;
