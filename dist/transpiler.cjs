...
`;
  ...
  javaProvableCounterInt(node) {
    ...
    const forStatement = _optionalChain([list, 'optionalAccess', _463 => _463.parent]);
    ...
    const scan = (n) => {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.BinaryExpression && JAVA_ASSIGNMENT_OPERATOR_KINDS.has(n.operatorToken.kind) && _optionalChain([n, 'access', _466 => _466.left, 'optionalAccess', _467 => _467.kind]) === _typescript2.default.SyntaxKind.Identifier && n.left.escapedText === name) {
        safe = false;
        return;
      }
      if ((n.kind === _typescript2.default.SyntaxKind.PostfixUnaryExpression || n.kind === _typescript2.default.SyntaxKind.PrefixUnaryExpression) && _optionalChain([n, 'access', _468 => _468.operand, 'optionalAccess', _469 => _469.kind]) === _typescript2.default.SyntaxKind.Identifier && n.operand.escapedText === name) {
        return;
      }
    ...
    if (enclosing) {
      ...
      const scanOuter = (n) => {
        ...
        if (n.kind === _typescript2.default.SyntaxKind.BinaryExpression && JAVA_ASSIGNMENT_OPERATOR_KINDS.has(n.operatorToken.kind) && _optionalChain([n, 'access', _470 => _470.left, 'optionalAccess', _471 => _471.kind]) === _typescript2.default.SyntaxKind.Identifier && n.left.escapedText === name) {
          safe = false;
          return;
        }
