...
`;
  ...
  goIsNativeAppendShape(receiverNode, pushNode) {
    if (_optionalChain([receiverNode, 'optionalAccess', _227 => _227.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    if (_optionalChain([pushNode, 'optionalAccess', _228 => _228.parent, 'optionalAccess', _229 => _229.kind]) !== _typescript2.default.SyntaxKind.ExpressionStatement) {
      return false;
    }
    if (pushNode.questionDotToken !== void 0 || _optionalChain([pushNode, 'access', _230 => _230.expression, 'optionalAccess', _231 => _231.questionDotToken]) !== void 0) {
      return false;
    }
    const access = pushNode.expression;
    if (_optionalChain([access, 'optionalAccess', _232 => _232.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || access.expression !== receiverNode || _optionalChain([access, 'access', _233 => _233.name, 'optionalAccess', _234 => _234.escapedText]) !== "push") {
      return false;
    }
    const args = pushNode.arguments;
    return _optionalChain([args, 'optionalAccess', _235 => _235.length]) === 1 && args[0].kind !== _typescript2.default.SyntaxKind.SpreadElement;
  }
  ...
  goNativeAppendReceiver(pushNode) {
    const receiver = _optionalChain([pushNode, 'optionalAccess', _236 => _236.expression, 'optionalAccess', _237 => _237.expression]);
