...
`;
  ...
  javaPrimitiveCounter(node) {
    if (_optionalChain([node, 'optionalAccess', _521 => _521.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    const symbol = this.getChecker().getSymbolAtLocation(node);
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _522 => _522.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _523 => _523.declarations, 'optionalAccess', _524 => _524[0]])));
    ...
    if (_optionalChain([list, 'optionalAccess', _525 => _525.kind]) !== _typescript2.default.SyntaxKind.VariableDeclarationList || list.declarations.length !== 1) {
      return false;
    }
    const forStatement = list.parent;
    if (_optionalChain([forStatement, 'optionalAccess', _526 => _526.kind]) !== _typescript2.default.SyntaxKind.ForStatement || forStatement.initializer !== list) {
      return false;
    }
    if (this.javaIntegerLiteralKind(declaration.initializer) === void 0) {
      return false;
    ...
    const incrementor = forStatement.incrementor;
    return _optionalChain([incrementor, 'optionalAccess', _527 => _527.kind]) === _typescript2.default.SyntaxKind.PostfixUnaryExpression && _optionalChain([incrementor, 'access', _528 => _528.operand, 'optionalAccess', _529 => _529.kind]) === _typescript2.default.SyntaxKind.Identifier && incrementor.operand.escapedText === node.escapedText;
  }
