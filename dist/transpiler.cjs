...
`;
  ...
  goSafeDictLocalArgs(initializer) {
    if (_optionalChain([initializer, 'optionalAccess', _238 => _238.kind]) !== _typescript2.default.SyntaxKind.CallExpression) {
      return void 0;
    }
    const callee = initializer.expression;
    if (_optionalChain([callee, 'optionalAccess', _239 => _239.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([callee, 'access', _240 => _240.name, 'optionalAccess', _241 => _241.escapedText]) !== "safeDict") {
      return void 0;
    }
    if (_optionalChain([callee, 'access', _242 => _242.expression, 'optionalAccess', _243 => _243.kind]) !== _typescript2.default.SyntaxKind.ThisKeyword) {
      return void 0;
    }
    const args = initializer.arguments;
    if (args.length === 3) {
      const fallback = args[2];
      if (_optionalChain([fallback, 'optionalAccess', _244 => _244.kind]) !== _typescript2.default.SyntaxKind.ObjectLiteralExpression || fallback.properties.length !== 0) {
        return void 0;
      }
    } else if (args.length !== 2) {
      return void 0;
  ...
  goSafeDictUseReadsTheMap(node) {
    ...
    switch (parent.kind) {
      case _typescript2.default.SyntaxKind.ElementAccessExpression: {
        ...
        if (_optionalChain([grandparent, 'optionalAccess', _245 => _245.kind]) === _typescript2.default.SyntaxKind.BinaryExpression && grandparent.left === parent) {
          return false;
        }
        if (_optionalChain([grandparent, 'optionalAccess', _246 => _246.kind]) === _typescript2.default.SyntaxKind.PostfixUnaryExpression || _optionalChain([grandparent, 'optionalAccess', _247 => _247.kind]) === _typescript2.default.SyntaxKind.PrefixUnaryExpression) {
          return false;
        }
        if (_optionalChain([grandparent, 'optionalAccess', _248 => _248.kind]) === _typescript2.default.SyntaxKind.DeleteExpression) {
          return false;
        }
        return true;
      }
      case _typescript2.default.SyntaxKind.BinaryExpression: {
        return _optionalChain([parent, 'access', _249 => _249.operatorToken, 'optionalAccess', _250 => _250.kind]) === _typescript2.default.SyntaxKind.InKeyword && parent.right === node;
      }
      case _typescript2.default.SyntaxKind.CallExpression: {
        if (parent.expression === node) {
          return false;
  ...
  goSafeDictLocalUnbox(declaration) {
    if (_optionalChain([declaration, 'optionalAccess', _251 => _251.kind]) !== _typescript2.default.SyntaxKind.VariableDeclaration || _optionalChain([declaration, 'access', _252 => _252.name, 'optionalAccess', _253 => _253.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
  ...
  goSafeDictLocalUnboxUncached(declaration) {
    ...
    const visit = (n) => {
      ...
      if ((n.kind === _typescript2.default.SyntaxKind.VariableDeclaration || n.kind === _typescript2.default.SyntaxKind.Parameter) && n !== declaration && _optionalChain([n, 'access', _257 => _257.name, 'optionalAccess', _258 => _258.kind]) === _typescript2.default.SyntaxKind.Identifier && n.name.escapedText === sourceName) {
        safe = false;
        return;
      }
  ...
  getGoLocalType(declaration, parsedValue) {
    ...
    const sourceName = _optionalChain([declaration, 'access', _259 => _259.name, 'optionalAccess', _260 => _260.escapedText]);
    if (sourceName === void 0) {
      return "any";
    }
    const scope = this.goEnclosingFunction(declaration);
  ...
  printVariableDeclarationList(node, identation) {
    const declaration = node.declarations[0];
    if (_optionalChain([declaration, 'optionalAccess', _261 => _261.name, 'access', _262 => _262.kind]) === _typescript2.default.SyntaxKind.ArrayBindingPattern) {
      const arrayBindingPattern = declaration.name;
      const arrayBindingPatternElements = arrayBindingPattern.elements;
      const parsedArrayBindingElements = arrayBindingPatternElements.map((e) => this.printNode(e.name, 0));
      const syntheticName = parsedArrayBindingElements.join("") + "Variable";
...
`;
    ...
    if (_optionalChain([declaration, 'optionalAccess', _263 => _263.initializer, 'optionalAccess', _264 => _264.kind]) === _typescript2.default.SyntaxKind.AwaitExpression) {
...
${this.getIden(identation)}PanicOnError(${parsedName})`;
    ...
    if (_optionalChain([node, 'optionalAccess', _265 => _265.parent, 'optionalAccess', _266 => _266.kind]) === _typescript2.default.SyntaxKind.FirstStatement) {
      if (isNew) {
...
${this.getIden(identation)}PanicOnError(${varName})`;
  ...
  printConstructorDeclaration(node, identation) {
    ...
    let superCallParams = "";
    let hasSuperCall = false;
    _optionalChain([node, 'access', _267 => _267.body, 'optionalAccess', _268 => _268.statements, 'access', _269 => _269.forEach, 'call', _270 => _270((statement) => {
      if (_typescript2.default.isExpressionStatement(statement)) {
        const expression = statement.expression;
        if (_typescript2.default.isCallExpression(expression)) {
          const expressionText = expression.expression.getText().trim();
  ...
  printThisElementAccesssIfNeeded(node, identation) {
    const isAsync = true;
    const elementAccess = node.expression;
    if (_optionalChain([elementAccess, 'optionalAccess', _271 => _271.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression) {
      if (_optionalChain([elementAccess, 'optionalAccess', _272 => _272.expression, 'optionalAccess', _273 => _273.kind]) === _typescript2.default.SyntaxKind.ThisKeyword) {
        let parsedArg = _optionalChain([node, 'access', _274 => _274.arguments, 'optionalAccess', _275 => _275.length]) > 0 ? this.printNode(node.arguments[0], identation).trimStart() : "";
        const propName = this.printNode(elementAccess.argumentExpression, 0);
        const wrapperOpen = isAsync ? this.UKNOWN_PROP_ASYNC_WRAPPER_OPEN : this.UKNOWN_PROP_WRAPPER_OPEN;
        const wrapperClose = isAsync ? this.UNKOWN_PROP_ASYNC_WRAPPER_CLOSE : this.UNKOWN_PROP_WRAPPER_CLOSE;
        parsedArg = parsedArg ? ", " + parsedArg : "";
  ...
  printDynamicCall(node, identation) {
    const elementAccess = node.expression;
    if (_optionalChain([elementAccess, 'optionalAccess', _276 => _276.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression) {
      const argumentDepth = this.goExprDepth + (_optionalChain([node, 'access', _277 => _277.arguments, 'optionalAccess', _278 => _278.length]) > 0 ? 1 : 0);
      const parsedArg = _optionalChain([node, 'access', _279 => _279.arguments, 'optionalAccess', _280 => _280.length]) > 0 ? node.arguments.map((n) => this.goWithExprDepth(argumentDepth, () => this.printNode(n, identation).trimStart())).join(", ") : "";
  ...
  printWrappedUnknownThisProperty(node, identation = 0) {
    const type = this.getChecker().getResolvedSignature(node);
    if (_optionalChain([type, 'optionalAccess', _281 => _281.declaration]) === void 0) {
      const argumentDepth = this.goExprDepth + (_optionalChain([node, 'access', _282 => _282.arguments, 'optionalAccess', _283 => _283.length]) > 0 ? 1 : 0);
      let parsedArguments = _optionalChain([node, 'access', _284 => _284.arguments, 'optionalAccess', _285 => _285.map, 'call', _286 => _286((a) => this.goWithExprDepth(argumentDepth, () => this.printNode(a, identation).trimStart())), 'access', _287 => _287.join, 'call', _288 => _288(", ")]);
      parsedArguments = parsedArguments ? parsedArguments : "";
      const propName = _optionalChain([node, 'access', _289 => _289.expression, 'optionalAccess', _290 => _290.name, 'access', _291 => _291.escapedText]);
  ...
  printOutOfOrderCallExpressionIfAny(node, identation) {
    if (node.expression.kind === _typescript2.default.SyntaxKind.PropertyAccessExpression) {
      ...
      const leftSide = _optionalChain([node, 'access', _292 => _292.expression, 'optionalAccess', _293 => _293.expression]);
      const leftSideText = leftSide ? this.printNode(leftSide, 0) : void 0;
      if (leftSideText === this.THIS_TOKEN || leftSide.getFullText().indexOf("(this as any)") > -1) {
...
`;
  ...
  goScalarFamily(node) {
    if (_optionalChain([node, 'optionalAccess', _294 => _294.kind]) === _typescript2.default.SyntaxKind.Identifier && this.goDeclaredTypeOfIdentifier(node) === void 0) {
      let decl;
      try {
        decl = _optionalChain([this, 'access', _295 => _295.getChecker, 'call', _296 => _296(), 'access', _297 => _297.getSymbolAtLocation, 'call', _298 => _298(node), 'optionalAccess', _299 => _299.valueDeclaration]);
      } catch (e) {
        decl = void 0;
      }
  ...
  goIsAnyBoxExpression(node, printedText) {
    if (_optionalChain([node, 'optionalAccess', _300 => _300.kind]) === _typescript2.default.SyntaxKind.Identifier) {
      ...
      const isBinding = _optionalChain([decl, 'optionalAccess', _302 => _302.kind]) === _typescript2.default.SyntaxKind.Parameter || _optionalChain([decl, 'optionalAccess', _303 => _303.kind]) === _typescript2.default.SyntaxKind.VariableDeclaration;
      if (!isBinding) {
        return false;
      }
  ...
  goAnyLocalHoldsPointer(decl) {
    if (_optionalChain([decl, 'optionalAccess', _305 => _305.kind]) !== _typescript2.default.SyntaxKind.VariableDeclaration || _optionalChain([decl, 'access', _306 => _306.name, 'optionalAccess', _307 => _307.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    ...
    const isPointerInit = (expr) => {
      while (_optionalChain([expr, 'optionalAccess', _308 => _308.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
        expr = expr.expression;
      }
      if (_optionalChain([expr, 'optionalAccess', _309 => _309.kind]) !== _typescript2.default.SyntaxKind.CallExpression) {
        return false;
      }
      const callee = expr.expression;
      if (_optionalChain([callee, 'optionalAccess', _310 => _310.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([callee, 'access', _311 => _311.expression, 'optionalAccess', _312 => _312.kind]) !== _typescript2.default.SyntaxKind.ThisKeyword) {
        return false;
      }
      const name = _optionalChain([callee, 'access', _313 => _313.name, 'optionalAccess', _314 => _314.escapedText]);
    ...
    if (!holds) {
      ...
      const visit = (n) => {
        ...
        if (n.kind === _typescript2.default.SyntaxKind.BinaryExpression && n.operatorToken.kind === _typescript2.default.SyntaxKind.EqualsToken && _optionalChain([n, 'access', _315 => _315.left, 'optionalAccess', _316 => _316.kind]) === _typescript2.default.SyntaxKind.Identifier && n.left.escapedText === name && isPointerInit(n.right)) {
          holds = true;
          return;
        }
        _typescript2.default.forEachChild(n, visit);
