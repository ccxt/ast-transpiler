...
`;
  ...
  csharpDeclaredReadType(node) {
    ...
    const declaration = _optionalChain([this, 'access', _184 => _184.getChecker, 'call', _185 => _185(), 'access', _186 => _186.getSymbolAtLocation, 'call', _187 => _187(node), 'optionalAccess', _188 => _188.valueDeclaration]);
    if (declaration === void 0 || !_typescript2.default.isVariableDeclaration(declaration) || _optionalChain([declaration, 'access', _189 => _189.name, 'optionalAccess', _190 => _190.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
    if (!this.csharpLocalTypes.has(declaration)) {
      this.csharpLocalTypes.set(declaration, this.getCSharpLocalType(declaration));
  ...
  csharpNullableBoolCondition(node) {
    let value = node;
    while (_optionalChain([value, 'optionalAccess', _191 => _191.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
      value = value.expression;
    }
    if (_optionalChain([value, 'optionalAccess', _192 => _192.kind]) !== _typescript2.default.SyntaxKind.Identifier || this.csharpDeclaredReadType(value) !== "bool?") {
      return void 0;
    }
