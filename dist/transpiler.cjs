...
`;
  ...
  nativeSliceCallIfProvable(node, name) {
    const args = _nullishCoalesce(_optionalChain([node, 'optionalAccess', _516 => _516.arguments]), () => ( []));
    if (args.length < 1 || args.length > 2) {
      return void 0;
    }
    ...
    const receiverExpression = _typescript2.default.isPropertyAccessExpression(_optionalChain([node, 'optionalAccess', _517 => _517.expression])) ? node.expression.expression : void 0;
