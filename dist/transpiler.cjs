...
${this.getIden(level)}}()`;
  ...
  goNativeCondition(node) {
    ...
    if (_optionalChain([node, 'optionalAccess', _320 => _320.kind]) === _typescript2.default.SyntaxKind.CallExpression && (printed === "true" || printed === "false")) {
      return printed;
    }
...
${this.getIden(identation)}${returnStatement}`;
  ...
  goIdentifierUsedElsewhere(nameNode) {
    ...
    const visit = (n) => {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.Identifier && n.escapedText === name && n !== nameNode) {
        const parent = n.parent;
        const isDeclarationName = _optionalChain([parent, 'optionalAccess', _354 => _354.kind]) === _typescript2.default.SyntaxKind.VariableDeclaration && parent.name === n;
        const isPropertyName = _optionalChain([parent, 'optionalAccess', _355 => _355.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression && parent.name === n;
  ...
  goLocalHoldsOnlyArrays(nameNode) {
    ...
    const visit = (n) => {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.Identifier && n.escapedText === name && n !== declaration.name && n !== nameNode) {
        const parent = n.parent;
        if (_optionalChain([parent, 'optionalAccess', _358 => _358.kind]) === _typescript2.default.SyntaxKind.VariableDeclaration && parent.name === n) {
          return;
        }
        if (this.goRebindingTargetOf(n) !== void 0) {
          safe = false;
  ...
  printNativeIsArray(node, parsedArg) {
    ...
    const argNode = _optionalChain([node, 'access', _359 => _359.arguments, 'optionalAccess', _360 => _360[0]]);
    if (_optionalChain([argNode, 'optionalAccess', _361 => _361.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
