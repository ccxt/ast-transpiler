...
`;
  ...
  rustCallPrintsBool(node) {
    if (_optionalChain([node, 'optionalAccess', _530 => _530.kind]) !== SyntaxKind4.CallExpression) {
      return false;
    }
    const callee = node.expression;
    let name;
    if (_optionalChain([callee, 'optionalAccess', _531 => _531.kind]) === SyntaxKind4.Identifier) {
      name = callee.escapedText;
    } else if (_optionalChain([callee, 'optionalAccess', _532 => _532.kind]) === SyntaxKind4.PropertyAccessExpression) {
      name = _optionalChain([callee, 'access', _533 => _533.name, 'optionalAccess', _534 => _534.escapedText]);
    }
