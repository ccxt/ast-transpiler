...
`;
  ...
  goSafeListLocalArgs(initializer) {
    if (_optionalChain([initializer, 'optionalAccess', _460 => _460.kind]) !== _typescript2.default.SyntaxKind.CallExpression) {
      return void 0;
    }
    const callee = initializer.expression;
    if (_optionalChain([callee, 'optionalAccess', _461 => _461.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([callee, 'access', _462 => _462.name, 'optionalAccess', _463 => _463.escapedText]) !== "safeList") {
      return void 0;
    }
    if (_optionalChain([callee, 'access', _464 => _464.expression, 'optionalAccess', _465 => _465.kind]) !== _typescript2.default.SyntaxKind.ThisKeyword) {
      return void 0;
    }
    const args = initializer.arguments;
    if (args.length === 3) {
      const fallback = args[2];
      if (_optionalChain([fallback, 'optionalAccess', _466 => _466.kind]) !== _typescript2.default.SyntaxKind.ArrayLiteralExpression || fallback.elements.length !== 0) {
        return void 0;
      }
    } else if (args.length !== 2) {
      return void 0;
  ...
  goSafeListUseReadsTheList(node) {
    const parent = node.parent;
    if (parent === void 0) {
      return false;
    }
    switch (parent.kind) {
      case _typescript2.default.SyntaxKind.PropertyAccessExpression: {
        ...
        if (_optionalChain([parent, 'access', _467 => _467.name, 'optionalAccess', _468 => _468.escapedText]) === "length") {
          return true;
        }
        return _optionalChain([parent, 'access', _469 => _469.name, 'optionalAccess', _470 => _470.escapedText]) === "push" && this.goIsNativeAppendShape(node, parent.parent);
      }
      case _typescript2.default.SyntaxKind.ElementAccessExpression: {
        if (parent.expression !== node) {
          return false;
        }
        return !this.isGoElementAccessAssignmentTarget(parent);
      }
      case _typescript2.default.SyntaxKind.CallExpression: {
        if (parent.expression === node) {
          return false;
  ...
  goSafeListLocalUnbox(declaration) {
    if (_optionalChain([declaration, 'optionalAccess', _471 => _471.kind]) !== _typescript2.default.SyntaxKind.VariableDeclaration || _optionalChain([declaration, 'access', _472 => _472.name, 'optionalAccess', _473 => _473.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
    if (_optionalChain([declaration, 'access', _474 => _474.parent, 'optionalAccess', _475 => _475.parent, 'optionalAccess', _476 => _476.kind]) !== _typescript2.default.SyntaxKind.FirstStatement) {
      return void 0;
    }
    if (this.goSafeListLocalUnboxCache.has(declaration)) {
      return this.goSafeListLocalUnboxCache.get(declaration);
    }
    this.goSafeListLocalUnboxCache.set(declaration, void 0);
    let result;
    try {
  ...
  goSafeListLocalUnboxUncached(declaration) {
    ...
    const visit = (n) => {
      ...
      if ((n.kind === _typescript2.default.SyntaxKind.VariableDeclaration || n.kind === _typescript2.default.SyntaxKind.Parameter) && n !== declaration && _optionalChain([n, 'access', _477 => _477.name, 'optionalAccess', _478 => _478.kind]) === _typescript2.default.SyntaxKind.Identifier && n.name.escapedText === sourceName) {
        safe = false;
        return;
      }
      if (n.kind === _typescript2.default.SyntaxKind.Identifier && n.escapedText === sourceName && n !== declaration.name) {
  ...
  goSafeListUnboxValue(declaration, identation) {
    if (this.goSafeListLocalUnbox(declaration) !== GO_SAFE_LIST_LOCAL_TYPE) {
      return void 0;
    }
    const args = this.goSafeListLocalArgs(declaration.initializer);
    const container = this.printNode(args.container, identation);
    const key = this.printNode(args.key, 0);
  ...
  getGoLocalType(declaration, parsedValue) {
    ...
    if (goType === void 0) {
      return _nullishCoalesce(_nullishCoalesce(this.goSafeDictLocalUnbox(declaration), () => ( this.goSafeListLocalUnbox(declaration))), () => ( "any"));
    }
    const sourceName = _optionalChain([declaration, 'access', _479 => _479.name, 'optionalAccess', _480 => _480.escapedText]);
    if (sourceName === void 0) {
      return "any";
    }
    const scope = this.goEnclosingFunction(declaration);
  ...
  printVariableDeclarationList(node, identation) {
    const declaration = node.declarations[0];
    if (_optionalChain([declaration, 'optionalAccess', _481 => _481.name, 'access', _482 => _482.kind]) === _typescript2.default.SyntaxKind.ArrayBindingPattern) {
      const arrayBindingPattern = declaration.name;
      const arrayBindingPatternElements = arrayBindingPattern.elements;
      const parsedArrayBindingElements = arrayBindingPatternElements.map((e) => this.printNode(e.name, 0));
      const syntheticName = parsedArrayBindingElements.join("") + "Variable";
...
`;
    ...
    if (_optionalChain([declaration, 'optionalAccess', _483 => _483.initializer, 'optionalAccess', _484 => _484.kind]) === _typescript2.default.SyntaxKind.AwaitExpression) {
...
${this.getIden(identation)}PanicOnError(${parsedName})`;
    ...
    if (_optionalChain([node, 'optionalAccess', _485 => _485.parent, 'optionalAccess', _486 => _486.kind]) === _typescript2.default.SyntaxKind.FirstStatement) {
      if (isNew) {
      ...
      const unboxValue = declaredType === GO_SAFE_DICT_LOCAL_TYPE ? this.goSafeDictUnboxValue(declaration, identation) : declaredType === GO_SAFE_LIST_LOCAL_TYPE ? this.goSafeListUnboxValue(declaration, identation) : void 0;
      const declaredValue = unboxValue !== void 0 ? unboxValue : parsedValue.trimStart();
...
${this.getIden(identation)}PanicOnError(${varName})`;
  ...
  printConstructorDeclaration(node, identation) {
    ...
    let superCallParams = "";
    let hasSuperCall = false;
    _optionalChain([node, 'access', _487 => _487.body, 'optionalAccess', _488 => _488.statements, 'access', _489 => _489.forEach, 'call', _490 => _490((statement) => {
      if (_typescript2.default.isExpressionStatement(statement)) {
        const expression = statement.expression;
        if (_typescript2.default.isCallExpression(expression)) {
          const expressionText = expression.expression.getText().trim();
  ...
  printThisElementAccesssIfNeeded(node, identation) {
    const isAsync = true;
    const elementAccess = node.expression;
    if (_optionalChain([elementAccess, 'optionalAccess', _491 => _491.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression) {
      if (_optionalChain([elementAccess, 'optionalAccess', _492 => _492.expression, 'optionalAccess', _493 => _493.kind]) === _typescript2.default.SyntaxKind.ThisKeyword) {
        let parsedArg = _optionalChain([node, 'access', _494 => _494.arguments, 'optionalAccess', _495 => _495.length]) > 0 ? this.printNode(node.arguments[0], identation).trimStart() : "";
        const propName = this.printNode(elementAccess.argumentExpression, 0);
        const wrapperOpen = isAsync ? this.UKNOWN_PROP_ASYNC_WRAPPER_OPEN : this.UKNOWN_PROP_WRAPPER_OPEN;
        const wrapperClose = isAsync ? this.UNKOWN_PROP_ASYNC_WRAPPER_CLOSE : this.UNKOWN_PROP_WRAPPER_CLOSE;
        parsedArg = parsedArg ? ", " + parsedArg : "";
  ...
  printDynamicCall(node, identation) {
    const elementAccess = node.expression;
    if (_optionalChain([elementAccess, 'optionalAccess', _496 => _496.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression) {
      const argumentDepth = this.goExprDepth + (_optionalChain([node, 'access', _497 => _497.arguments, 'optionalAccess', _498 => _498.length]) > 0 ? 1 : 0);
      const parsedArg = _optionalChain([node, 'access', _499 => _499.arguments, 'optionalAccess', _500 => _500.length]) > 0 ? node.arguments.map((n) => this.goWithExprDepth(argumentDepth, () => this.printNode(n, identation).trimStart())).join(", ") : "";
  ...
  printWrappedUnknownThisProperty(node, identation = 0) {
    const type = this.getChecker().getResolvedSignature(node);
    if (_optionalChain([type, 'optionalAccess', _501 => _501.declaration]) === void 0) {
      const argumentDepth = this.goExprDepth + (_optionalChain([node, 'access', _502 => _502.arguments, 'optionalAccess', _503 => _503.length]) > 0 ? 1 : 0);
      let parsedArguments = _optionalChain([node, 'access', _504 => _504.arguments, 'optionalAccess', _505 => _505.map, 'call', _506 => _506((a) => this.goWithExprDepth(argumentDepth, () => this.printNode(a, identation).trimStart())), 'access', _507 => _507.join, 'call', _508 => _508(", ")]);
      parsedArguments = parsedArguments ? parsedArguments : "";
      const propName = _optionalChain([node, 'access', _509 => _509.expression, 'optionalAccess', _510 => _510.name, 'access', _511 => _511.escapedText]);
  ...
  printOutOfOrderCallExpressionIfAny(node, identation) {
    if (node.expression.kind === _typescript2.default.SyntaxKind.PropertyAccessExpression) {
      ...
      const leftSide = _optionalChain([node, 'access', _512 => _512.expression, 'optionalAccess', _513 => _513.expression]);
      const leftSideText = leftSide ? this.printNode(leftSide, 0) : void 0;
      if (leftSideText === this.THIS_TOKEN || leftSide.getFullText().indexOf("(this as any)") > -1) {
...
`;
  ...
  goScalarFamily(node) {
    if (_optionalChain([node, 'optionalAccess', _514 => _514.kind]) === _typescript2.default.SyntaxKind.Identifier && this.goDeclaredTypeOfIdentifier(node) === void 0) {
      let decl;
      try {
        decl = _optionalChain([this, 'access', _515 => _515.getChecker, 'call', _516 => _516(), 'access', _517 => _517.getSymbolAtLocation, 'call', _518 => _518(node), 'optionalAccess', _519 => _519.valueDeclaration]);
      } catch (e) {
        decl = void 0;
      }
  ...
  goObjectBoxParameter(node) {
    if (_optionalChain([node, 'optionalAccess', _520 => _520.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    let symbol;
    try {
    ...
    if (_optionalChain([symbol, 'optionalAccess', _521 => _521.valueDeclaration, 'optionalAccess', _522 => _522.kind]) !== _typescript2.default.SyntaxKind.Parameter) {
      return false;
    }
    let type;
    try {
  ...
  goTypeIsNilComparableObject(type) {
    ...
    const declaration = _nullishCoalesce(_optionalChain([type, 'access', _523 => _523.symbol, 'optionalAccess', _524 => _524.valueDeclaration]), () => ( _optionalChain([type, 'access', _525 => _525.symbol, 'optionalAccess', _526 => _526.declarations, 'optionalAccess', _527 => _527[0]])));
    return _optionalChain([declaration, 'optionalAccess', _528 => _528.kind]) !== _typescript2.default.SyntaxKind.ClassDeclaration;
  }
  ...
  goIsAnyBoxExpression(node, printedText) {
    if (_optionalChain([node, 'optionalAccess', _529 => _529.kind]) === _typescript2.default.SyntaxKind.Identifier) {
      let symbol;
      try {
        symbol = this.getChecker().getSymbolAtLocation(node);
      } catch (e) {
      ...
      const decl = _optionalChain([symbol, 'optionalAccess', _530 => _530.valueDeclaration]);
      const isBinding = _optionalChain([decl, 'optionalAccess', _531 => _531.kind]) === _typescript2.default.SyntaxKind.Parameter || _optionalChain([decl, 'optionalAccess', _532 => _532.kind]) === _typescript2.default.SyntaxKind.VariableDeclaration;
      if (!isBinding) {
        return false;
      }
    ...
    if (_optionalChain([node, 'optionalAccess', _533 => _533.kind]) === _typescript2.default.SyntaxKind.CallExpression) {
  ...
  goBoxedElementRead(node, printedText) {
    while (_optionalChain([node, 'optionalAccess', _534 => _534.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
      node = node.expression;
    }
    if (_optionalChain([node, 'optionalAccess', _535 => _535.kind]) !== _typescript2.default.SyntaxKind.ElementAccessExpression) {
      return false;
    }
    if (GO_ANY_BOX_CALLS.indexOf(this.goPrintedCallee(printedText)) >= 0) {
      return true;
  ...
  goAnyLocalHoldsPointer(decl) {
    if (_optionalChain([decl, 'optionalAccess', _536 => _536.kind]) !== _typescript2.default.SyntaxKind.VariableDeclaration || _optionalChain([decl, 'access', _537 => _537.name, 'optionalAccess', _538 => _538.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    ...
    const isPointerInit = (expr) => {
      while (_optionalChain([expr, 'optionalAccess', _539 => _539.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
        expr = expr.expression;
      }
      if (_optionalChain([expr, 'optionalAccess', _540 => _540.kind]) !== _typescript2.default.SyntaxKind.CallExpression) {
        return false;
      }
      const callee = expr.expression;
      if (_optionalChain([callee, 'optionalAccess', _541 => _541.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([callee, 'access', _542 => _542.expression, 'optionalAccess', _543 => _543.kind]) !== _typescript2.default.SyntaxKind.ThisKeyword) {
        return false;
      }
      const name = _optionalChain([callee, 'access', _544 => _544.name, 'optionalAccess', _545 => _545.escapedText]);
    ...
    if (!holds) {
      ...
      const visit = (n) => {
        ...
        if (n.kind === _typescript2.default.SyntaxKind.BinaryExpression && n.operatorToken.kind === _typescript2.default.SyntaxKind.EqualsToken && _optionalChain([n, 'access', _546 => _546.left, 'optionalAccess', _547 => _547.kind]) === _typescript2.default.SyntaxKind.Identifier && n.left.escapedText === name && isPointerInit(n.right)) {
          holds = true;
          return;
        }
        _typescript2.default.forEachChild(n, visit);
  ...
  goAstCalleeName(call) {
    const callee = _optionalChain([call, 'optionalAccess', _548 => _548.expression]);
    if (_optionalChain([callee, 'optionalAccess', _549 => _549.kind]) === _typescript2.default.SyntaxKind.Identifier) {
      const name = callee.escapedText;
      return typeof name === "string" && name.length > 0 ? name.charAt(0).toUpperCase() + name.substring(1) : void 0;
    }
    if (_optionalChain([callee, 'optionalAccess', _550 => _550.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression && _optionalChain([callee, 'access', _551 => _551.expression, 'optionalAccess', _552 => _552.kind]) === _typescript2.default.SyntaxKind.ThisKeyword) {
      const name = _optionalChain([callee, 'access', _553 => _553.name, 'optionalAccess', _554 => _554.escapedText]);
  ...
  goAwaitedCallIsImplicitEndpoint(expression) {
    let call = expression;
    while (_optionalChain([call, 'optionalAccess', _555 => _555.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
      call = call.expression;
    }
    if (_optionalChain([call, 'optionalAccess', _556 => _556.kind]) !== _typescript2.default.SyntaxKind.CallExpression) {
      return false;
    }
    const callee = call.expression;
    if (_optionalChain([callee, 'optionalAccess', _557 => _557.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([callee, 'access', _558 => _558.expression, 'optionalAccess', _559 => _559.kind]) !== _typescript2.default.SyntaxKind.ThisKeyword) {
      return false;
    }
    const nameNode = callee.name;
    if (_optionalChain([nameNode, 'optionalAccess', _560 => _560.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    let symbol;
    try {
    ...
    const declarations = _optionalChain([symbol, 'optionalAccess', _561 => _561.declarations]);
    if (!declarations || declarations.length === 0) {
      return false;
    }
    return declarations.every((d) => d.kind === _typescript2.default.SyntaxKind.MethodSignature && d.body === void 0 && _optionalChain([d, 'access', _562 => _562.parent, 'optionalAccess', _563 => _563.kind]) === _typescript2.default.SyntaxKind.InterfaceDeclaration);
  }
  ...
  goIsNonPointerValueSource(expr) {
    while (_optionalChain([expr, 'optionalAccess', _564 => _564.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
      expr = expr.expression;
    }
    if (expr === void 0) {
      return true;
  ...
  goAnyLocalHoldsNonPointer(decl) {
    if (_optionalChain([decl, 'optionalAccess', _565 => _565.kind]) !== _typescript2.default.SyntaxKind.VariableDeclaration || _optionalChain([decl, 'access', _566 => _566.name, 'optionalAccess', _567 => _567.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    ...
    if (holds) {
      ...
      if (scope === void 0) {
        ...
        const visit = (n) => {
          ...
          if (n.kind === _typescript2.default.SyntaxKind.BinaryExpression && _optionalChain([n, 'access', _568 => _568.operatorToken, 'optionalAccess', _569 => _569.kind]) === _typescript2.default.SyntaxKind.EqualsToken && this.goAssignmentWritesName(n.left, name) && !this.goIsNonPointerValueSource(n.right)) {
            holds = false;
            return;
          }
          _typescript2.default.forEachChild(n, visit);
  ...
  goAssignmentWritesName(left, name) {
    if (_optionalChain([left, 'optionalAccess', _570 => _570.kind]) === _typescript2.default.SyntaxKind.Identifier) {
      return left.escapedText === name;
    }
    if (_optionalChain([left, 'optionalAccess', _571 => _571.kind]) === _typescript2.default.SyntaxKind.ArrayLiteralExpression) {
      return left.elements.some((e) => _optionalChain([e, 'optionalAccess', _572 => _572.kind]) === _typescript2.default.SyntaxKind.Identifier && e.escapedText === name);
    }
    return false;
  }
  // the variable declaration an identifier resolves to, when it is one
  goAnyBoxLocalDeclaration(node) {
    if (_optionalChain([node, 'optionalAccess', _573 => _573.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
    let symbol;
    try {
    ...
    return _optionalChain([symbol, 'optionalAccess', _574 => _574.valueDeclaration]);
  }
  goScalarFamilyOfType(type, allowNil = false) {
    if (type === void 0) {
      return void 0;
    }
    const alias = _optionalChain([type, 'access', _575 => _575.aliasSymbol, 'optionalAccess', _576 => _576.escapedName]);
    if (!allowNil) {
      switch (alias) {
        case "Str":
        case "Int":
  ...
  goDeclaredTypeOfIdentifier(node) {
    if (_optionalChain([node, 'optionalAccess', _577 => _577.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
    let symbol;
    try {
    ...
    const decl = _optionalChain([symbol, 'optionalAccess', _578 => _578.valueDeclaration]);
    if (decl === void 0 || decl.kind !== _typescript2.default.SyntaxKind.VariableDeclaration) {
      return void 0;
    }
    if (decl.initializer === void 0 || _optionalChain([decl, 'access', _579 => _579.name, 'optionalAccess', _580 => _580.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
  ...
  goPointerTypeOfExpression(node, printedText) {
    ...
    if (_optionalChain([node, 'optionalAccess', _581 => _581.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression) {
    ...
    if (_optionalChain([node, 'optionalAccess', _582 => _582.kind]) === _typescript2.default.SyntaxKind.CallExpression) {
      const goType = this.goTypeOfInitializer(node, printedText);
  ...
  goElementAssignmentContainerType(node, printedText) {
    ...
    if (_optionalChain([node, 'optionalAccess', _583 => _583.kind]) === _typescript2.default.SyntaxKind.CallExpression) {
  ...
  goFieldContainerTypeNative(node) {
    if (_optionalChain([node, 'optionalAccess', _584 => _584.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([node, 'access', _585 => _585.expression, 'optionalAccess', _586 => _586.kind]) !== _typescript2.default.SyntaxKind.ThisKeyword) {
      return void 0;
    }
    const name = _optionalChain([node, 'access', _587 => _587.name, 'optionalAccess', _588 => _588.escapedText]);
  ...
  goSliceIndexProvablyInRange(node, indexNode) {
    if (_optionalChain([node, 'optionalAccess', _589 => _589.kind]) !== _typescript2.default.SyntaxKind.Identifier || _optionalChain([indexNode, 'optionalAccess', _590 => _590.kind]) !== _typescript2.default.SyntaxKind.NumericLiteral) {
      return false;
    }
    ...
    const decl = _optionalChain([symbol, 'optionalAccess', _591 => _591.valueDeclaration]);
    if (decl === void 0 || decl.kind !== _typescript2.default.SyntaxKind.VariableDeclaration || _optionalChain([decl, 'access', _592 => _592.name, 'optionalAccess', _593 => _593.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    const initializer = decl.initializer;
    if (_optionalChain([initializer, 'optionalAccess', _594 => _594.kind]) !== _typescript2.default.SyntaxKind.ArrayLiteralExpression) {
      return false;
    }
  ...
  goRebindingTargetOf(identifier) {
    let node = identifier;
    let parent = node.parent;
    while (_optionalChain([parent, 'optionalAccess', _595 => _595.kind]) === _typescript2.default.SyntaxKind.ArrayLiteralExpression) {
      node = parent;
      parent = parent.parent;
    }
    if (_optionalChain([parent, 'optionalAccess', _596 => _596.kind]) === _typescript2.default.SyntaxKind.ForOfStatement && parent.initializer === node) {
      return parent;
    }
    if (_optionalChain([parent, 'optionalAccess', _597 => _597.kind]) === _typescript2.default.SyntaxKind.BinaryExpression && parent.left === node) {
      const op = parent.operatorToken.kind;
  ...
  goPrintedTypeOfExpression(node, printedText) {
    const inner = _optionalChain([node, 'optionalAccess', _598 => _598.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression ? node.expression : node;
    if (_optionalChain([inner, 'optionalAccess', _599 => _599.kind]) === _typescript2.default.SyntaxKind.Identifier) {
  ...
  goLengthFeedsArithmeticClassifier(lengthNode) {
    let current = _optionalChain([lengthNode, 'optionalAccess', _600 => _600.parent]);
    for (let i = 0; i < 16 && current !== void 0; i++) {
      if (current.kind === _typescript2.default.SyntaxKind.BinaryExpression) {
        const op = _optionalChain([current, 'access', _601 => _601.operatorToken, 'optionalAccess', _602 => _602.kind]);
...
${this.getIden(level)}}()`;
  ...
  goTernaryArmType(node, printedText) {
    ...
    const inner = _optionalChain([node, 'optionalAccess', _603 => _603.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression ? node.expression : node;
    switch (_optionalChain([inner, 'optionalAccess', _604 => _604.kind])) {
      case _typescript2.default.SyntaxKind.StringLiteral:
      case _typescript2.default.SyntaxKind.NoSubstitutionTemplateLiteral:
        return "string";
      case _typescript2.default.SyntaxKind.TrueKeyword:
  ...
  printInlineInOp(dictNode, keyNode, dictText, keyText) {
    ...
    if (this.goPrintedTypeOfExpression(keyNode, keyText) !== "string") {
      if (_optionalChain([keyNode, 'optionalAccess', _605 => _605.kind]) !== _typescript2.default.SyntaxKind.Identifier || this.goDeclaredTypeOfIdentifier(keyNode) !== "*string") {
        return void 0;
      }
...
${this.getIden(level)}}()`;
  ...
  printInlineOpNeg(node, printedText) {
    ...
    if (/^\d+(\.\d+)?([eE][+-]?\d+)?$/.test(printedText.trim())) {
      ...
      const parent = node.parent;
      if (_optionalChain([parent, 'optionalAccess', _606 => _606.kind]) === _typescript2.default.SyntaxKind.CallExpression) {
        const callee = this.printNode(parent.expression, 0);
        if (this.comparisonHelpers.indexOf(callee) >= 0) {
          return `-${printedText}`;
        }
      }
      if (_optionalChain([parent, 'optionalAccess', _607 => _607.kind]) === _typescript2.default.SyntaxKind.BinaryExpression) {
        const op = parent.operatorToken.kind;
  ...
  printInlineTruthy(node) {
    if (_optionalChain([node, 'optionalAccess', _608 => _608.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
    const goType = this.goDeclaredTypeOfIdentifier(node);
    if (goType === void 0) {
  ...
  goNativeCondition(node) {
    if (_optionalChain([node, 'optionalAccess', _609 => _609.kind]) === _typescript2.default.SyntaxKind.Identifier) {
      return this.printInlineTruthy(node);
    }
    if (_optionalChain([node, 'optionalAccess', _610 => _610.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
    ...
    if (_optionalChain([node, 'optionalAccess', _611 => _611.kind]) === _typescript2.default.SyntaxKind.CallExpression && (printed === "true" || printed === "false")) {
      return printed;
    }
  ...
  goIsControlClauseCondition(node) {
    const parent = _optionalChain([node, 'optionalAccess', _612 => _612.parent]);
    switch (_optionalChain([parent, 'optionalAccess', _613 => _613.kind])) {
      case _typescript2.default.SyntaxKind.IfStatement:
      case _typescript2.default.SyntaxKind.WhileStatement:
  ...
  printCondition(node, identation) {
    ...
    const inNode = _optionalChain([node, 'optionalAccess', _614 => _614.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression ? node.expression : node;
    if (_optionalChain([inNode, 'optionalAccess', _615 => _615.kind]) === _typescript2.default.SyntaxKind.BinaryExpression && inNode.operatorToken.kind === _typescript2.default.SyntaxKind.InKeyword) {
  ...
  goDerefComparableWith(ptrNode, ptrText, otherNode) {
    const pointee = _optionalChain([this, 'access', _616 => _616.goPointerTypeOfExpression, 'call', _617 => _617(ptrNode, ptrText), 'optionalAccess', _618 => _618.substring, 'call', _619 => _619(1)]);
    if (pointee === void 0) {
      return false;
    }
    switch (_optionalChain([otherNode, 'optionalAccess', _620 => _620.kind])) {
      case _typescript2.default.SyntaxKind.StringLiteral:
      case _typescript2.default.SyntaxKind.NoSubstitutionTemplateLiteral:
  ...
  printInlineEquality(left, right, leftText, rightText, isEq) {
    ...
    const lRepeatable = _optionalChain([left, 'optionalAccess', _621 => _621.kind]) === _typescript2.default.SyntaxKind.Identifier;
    const rRepeatable = _optionalChain([right, 'optionalAccess', _622 => _622.kind]) === _typescript2.default.SyntaxKind.Identifier;
    ...
    const isLiteral = (node) => {
      switch (_optionalChain([node, 'optionalAccess', _623 => _623.kind])) {
        case _typescript2.default.SyntaxKind.StringLiteral:
        case _typescript2.default.SyntaxKind.NoSubstitutionTemplateLiteral:
        case _typescript2.default.SyntaxKind.TrueKeyword:
        case _typescript2.default.SyntaxKind.FalseKeyword:
    ...
    const isBoolLiteral = (node) => _optionalChain([node, 'optionalAccess', _624 => _624.kind]) === _typescript2.default.SyntaxKind.TrueKeyword || _optionalChain([node, 'optionalAccess', _625 => _625.kind]) === _typescript2.default.SyntaxKind.FalseKeyword;
  ...
  goLiteralTypedLocalKind(node) {
    ...
    const declaration = _optionalChain([symbol, 'optionalAccess', _626 => _626.valueDeclaration]);
    if (_optionalChain([declaration, 'optionalAccess', _627 => _627.kind]) !== _typescript2.default.SyntaxKind.VariableDeclaration || declaration.initializer === void 0) {
      return void 0;
    }
    const declarationList = declaration.parent;
    if (_optionalChain([declarationList, 'optionalAccess', _628 => _628.kind]) !== _typescript2.default.SyntaxKind.VariableDeclarationList || _optionalChain([declarationList, 'access', _629 => _629.parent, 'optionalAccess', _630 => _630.kind]) === _typescript2.default.SyntaxKind.FirstStatement) {
      return void 0;
    }
  ...
  goNumericLiteralKind(node) {
    ...
    const text = _optionalChain([node, 'optionalAccess', _631 => _631.text]);
    if (text === void 0) {
      return void 0;
    }
  ...
  goIsSignedNumericLiteral(node) {
    if (_optionalChain([node, 'optionalAccess', _632 => _632.kind]) !== _typescript2.default.SyntaxKind.PrefixUnaryExpression) {
      return false;
    }
    if (node.operator !== _typescript2.default.SyntaxKind.MinusToken && node.operator !== _typescript2.default.SyntaxKind.PlusToken) {
      return false;
    }
    return _optionalChain([node, 'access', _633 => _633.operand, 'optionalAccess', _634 => _634.kind]) === _typescript2.default.SyntaxKind.NumericLiteral;
  }
  // a numeric constant operand: an integer/float literal, with an optional sign
  goIsNumericConstant(node) {
    return _optionalChain([node, 'optionalAccess', _635 => _635.kind]) === _typescript2.default.SyntaxKind.NumericLiteral || this.goIsSignedNumericLiteral(node);
  }
  ...
  goNumericConstantValue(node) {
    ...
    if (_optionalChain([node, 'optionalAccess', _636 => _636.kind]) !== _typescript2.default.SyntaxKind.NumericLiteral) {
      return Number.NaN;
    }
  ...
  goOrderedComparisonPointerKind(node) {
    if (_optionalChain([node, 'optionalAccess', _637 => _637.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
    const declared = this.goDeclaredTypeOfIdentifier(node);
  ...
  goIsNonNilTestOf(node, ident) {
    if (_optionalChain([node, 'optionalAccess', _638 => _638.kind]) !== _typescript2.default.SyntaxKind.BinaryExpression) {
      return false;
    }
    const op = _optionalChain([node, 'access', _639 => _639.operatorToken, 'optionalAccess', _640 => _640.kind]);
  ...
  goIsNilLiteral(node) {
    if (_optionalChain([node, 'optionalAccess', _641 => _641.kind]) === _typescript2.default.SyntaxKind.NullKeyword || _optionalChain([node, 'optionalAccess', _642 => _642.kind]) === _typescript2.default.SyntaxKind.UndefinedKeyword) {
      return true;
    }
    return _optionalChain([node, 'optionalAccess', _643 => _643.kind]) === _typescript2.default.SyntaxKind.Identifier && this.goScalarFamily(node) === "nil";
  }
  goIsSameSymbol(a, b) {
    if (_optionalChain([a, 'optionalAccess', _644 => _644.kind]) !== _typescript2.default.SyntaxKind.Identifier || _optionalChain([b, 'optionalAccess', _645 => _645.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    try {
      const checker = this.getChecker();
  ...
  goHasEnclosingNilGuard(ident) {
    let declaration;
    try {
      declaration = _optionalChain([this, 'access', _646 => _646.getChecker, 'call', _647 => _647(), 'access', _648 => _648.getSymbolAtLocation, 'call', _649 => _649(ident), 'optionalAccess', _650 => _650.valueDeclaration]);
    } catch (e) {
      return false;
    }
    ...
    let child = ident;
    let parent = _optionalChain([ident, 'optionalAccess', _651 => _651.parent]);
    while (parent !== void 0) {
      ...
      if (kind === _typescript2.default.SyntaxKind.BinaryExpression) {
        const op = _optionalChain([parent, 'access', _652 => _652.operatorToken, 'optionalAccess', _653 => _653.kind]);
  ...
  goConditionProvesNonNil(condition, ident) {
    ...
    if (_optionalChain([inner, 'access', _654 => _654.operatorToken, 'optionalAccess', _655 => _655.kind]) !== _typescript2.default.SyntaxKind.AmpersandAmpersandToken) {
      return false;
    }
  ...
  goUnwrapParenthesizedNode(node) {
    let current = node;
    while (_optionalChain([current, 'optionalAccess', _656 => _656.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
  ...
  goIsPureComparisonOperand(node) {
    switch (_optionalChain([node, 'optionalAccess', _657 => _657.kind])) {
      case _typescript2.default.SyntaxKind.Identifier:
      case _typescript2.default.SyntaxKind.NumericLiteral:
      case _typescript2.default.SyntaxKind.StringLiteral:
      case _typescript2.default.SyntaxKind.NoSubstitutionTemplateLiteral:
  ...
  printParenthesizedExpression(node, identation) {
    const expression = node.expression;
    if (_optionalChain([expression, 'optionalAccess', _658 => _658.kind]) === _typescript2.default.SyntaxKind.AsExpression) {
      return this.getIden(identation) + this.printNode(expression, 0);
    }
    if (_optionalChain([expression, 'optionalAccess', _659 => _659.kind]) === _typescript2.default.SyntaxKind.ArrowFunction) {
      return "";
    }
  ...
  printExpressionStatement(node, identation) {
    if (_optionalChain([node, 'optionalAccess', _660 => _660.expression, 'optionalAccess', _661 => _661.kind]) === _typescript2.default.SyntaxKind.AsExpression) {
      node = node.expression;
    }
...
${this.getIden(identation)}PanicOnError(${returnRandName})`;
  ...
  printReturnStatement(node, identation) {
    ...
    if (_optionalChain([node, 'optionalAccess', _662 => _662.expression, 'optionalAccess', _663 => _663.kind]) === _typescript2.default.SyntaxKind.AsExpression) {
      node = node.expression;
    }
    if (_optionalChain([node, 'optionalAccess', _664 => _664.expression, 'optionalAccess', _665 => _665.kind]) === _typescript2.default.SyntaxKind.AwaitExpression) {
...
${this.getIden(identation)}${returnStatement}`;
  ...
  goIdentifierUsedElsewhere(nameNode) {
    ...
    const visit = (n) => {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.Identifier && n.escapedText === name && n !== nameNode) {
        const parent = n.parent;
        const isDeclarationName = _optionalChain([parent, 'optionalAccess', _666 => _666.kind]) === _typescript2.default.SyntaxKind.VariableDeclaration && parent.name === n;
        const isPropertyName = _optionalChain([parent, 'optionalAccess', _667 => _667.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression && parent.name === n;
  ...
  goLocalHoldsOnlyArrays(nameNode) {
    ...
    const declaration = _optionalChain([symbol, 'optionalAccess', _668 => _668.valueDeclaration]);
    if (_optionalChain([declaration, 'optionalAccess', _669 => _669.kind]) !== _typescript2.default.SyntaxKind.VariableDeclaration || declaration.initializer === void 0) {
      return false;
    }
    ...
    const visit = (n) => {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.Identifier && n.escapedText === name && n !== declaration.name && n !== nameNode) {
        const parent = n.parent;
        if (_optionalChain([parent, 'optionalAccess', _670 => _670.kind]) === _typescript2.default.SyntaxKind.VariableDeclaration && parent.name === n) {
          return;
        }
        if (this.goRebindingTargetOf(n) !== void 0) {
          safe = false;
  ...
  printNativeIsArray(node, parsedArg) {
    ...
    const argNode = _optionalChain([node, 'access', _671 => _671.arguments, 'optionalAccess', _672 => _672[0]]);
    if (_optionalChain([argNode, 'optionalAccess', _673 => _673.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
...
${this.getIden(level)}}()`;
  ...
  goPrintCallArgument(argument, printedText) {
    if (_optionalChain([argument, 'optionalAccess', _674 => _674.kind]) !== _typescript2.default.SyntaxKind.BinaryExpression || printedText === void 0) {
      return printedText;
    }
  ...
  printArrayPushCall(node, identation, name = void 0, parsedArg = void 0) {
    ...
    parsedArg = this.goPrintCallArgument(_optionalChain([node, 'access', _675 => _675.arguments, 'optionalAccess', _676 => _676[0]]), parsedArg);
    ...
    if (_optionalChain([name, 'optionalAccess', _677 => _677.startsWith, 'call', _678 => _678("GetValue")]) || /[\])]$/.test(_nullishCoalesce(name, () => ( "")))) {
...
${this.getIden(identation)}`;
  ...
  goIndexOfOperandType(node, printedText) {
    ...
    if (_optionalChain([node, 'optionalAccess', _679 => _679.kind]) === _typescript2.default.SyntaxKind.Identifier) {
  ...
  goFileKeepsFileLevelImports() {
    const fileName = _nullishCoalesce(_optionalChain([this, 'access', _680 => _680.getSrc, 'call', _681 => _681(), 'optionalAccess', _682 => _682.fileName]), () => ( ""));
  ...
  goNativeIndexOf(node, name, parsedArg) {
    ...
    const receiver = _optionalChain([node, 'optionalAccess', _683 => _683.expression, 'optionalAccess', _684 => _684.expression]);
    const target = _optionalChain([node, 'optionalAccess', _685 => _685.arguments, 'optionalAccess', _686 => _686[0]]);
    ...
    if (receiverType === "*string" && _optionalChain([receiver, 'optionalAccess', _687 => _687.kind]) === _typescript2.default.SyntaxKind.Identifier) {
      this.goFileStdlibImports.add("strings");
      const level = this.goStatementLevel;
      const body = this.getIden(level + 1);
...
${this.getIden(level)}}()`;
  ...
  printStartsWithCall(node, identation, name = void 0, parsedArg = void 0) {
    if (parsedArg !== void 0 && this.goNativeStringOperands([_optionalChain([node, 'access', _688 => _688.expression, 'optionalAccess', _689 => _689.expression]), _optionalChain([node, 'access', _690 => _690.arguments, 'optionalAccess', _691 => _691[0]])], [name, parsedArg], ["string", "string"])) {
  ...
  printEndsWithCall(node, identation, name = void 0, parsedArg = void 0) {
    if (parsedArg !== void 0 && this.goNativeStringOperands([_optionalChain([node, 'access', _692 => _692.expression, 'optionalAccess', _693 => _693.expression]), _optionalChain([node, 'access', _694 => _694.arguments, 'optionalAccess', _695 => _695[0]])], [name, parsedArg], ["string", "string"])) {
  ...
  printJoinCall(node, identation, name = void 0, parsedArg = void 0) {
    if (parsedArg !== void 0 && this.goNativeStringOperands([_optionalChain([node, 'access', _696 => _696.expression, 'optionalAccess', _697 => _697.expression]), _optionalChain([node, 'access', _698 => _698.arguments, 'optionalAccess', _699 => _699[0]])], [name, parsedArg], ["[]string", "string"])) {
  ...
  printSplitCall(node, identation, name = void 0, parsedArg = void 0) {
    if (parsedArg !== void 0 && this.goNativeStringOperands([_optionalChain([node, 'access', _700 => _700.expression, 'optionalAccess', _701 => _701.expression]), _optionalChain([node, 'access', _702 => _702.arguments, 'optionalAccess', _703 => _703[0]])], [name, parsedArg], ["string", "string"])) {
  ...
  printToStringCall(node, identation, name = void 0) {
    if (name !== void 0 && name.indexOf("\n") < 0) {
      const receiver = _optionalChain([node, 'optionalAccess', _704 => _704.expression, 'optionalAccess', _705 => _705.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression ? node.expression.expression : void 0;
  ...
  printToUpperCaseCall(node, identation, name = void 0) {
    if (this.goNativeStringOperands([_optionalChain([node, 'access', _706 => _706.expression, 'optionalAccess', _707 => _707.expression])], [name], ["string"])) {
  ...
  printToLowerCaseCall(node, identation, name = void 0) {
    if (this.goNativeStringOperands([_optionalChain([node, 'access', _708 => _708.expression, 'optionalAccess', _709 => _709.expression])], [name], ["string"])) {
  ...
  goSliceLiteralBound(node) {
    if (_optionalChain([node, 'optionalAccess', _710 => _710.kind]) === _typescript2.default.SyntaxKind.NumericLiteral) {
    ...
    if (_optionalChain([node, 'optionalAccess', _711 => _711.kind]) === _typescript2.default.SyntaxKind.PrefixUnaryExpression && node.operator === _typescript2.default.SyntaxKind.MinusToken) {
  ...
  goSliceReceiverType(node) {
    let receiverNode = _optionalChain([node, 'optionalAccess', _712 => _712.expression, 'optionalAccess', _713 => _713.expression]);
    while (_optionalChain([receiverNode, 'optionalAccess', _714 => _714.kind]) === _typescript2.default.SyntaxKind.AsExpression || _optionalChain([receiverNode, 'optionalAccess', _715 => _715.kind]) === _typescript2.default.SyntaxKind.NonNullExpression || _optionalChain([receiverNode, 'optionalAccess', _716 => _716.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
      receiverNode = receiverNode.expression;
    }
    if (_optionalChain([receiverNode, 'optionalAccess', _717 => _717.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return void 0;
    }
  ...
  goSliceLiteralBounds(node) {
    const args = _nullishCoalesce(_optionalChain([node, 'optionalAccess', _718 => _718.arguments]), () => ( []));
  ...
  goIsNativeSliceCall(node) {
    if (_optionalChain([node, 'optionalAccess', _719 => _719.kind]) !== _typescript2.default.SyntaxKind.CallExpression) {
      return false;
    }
    const callee = node.expression;
    if (_optionalChain([callee, 'optionalAccess', _720 => _720.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([callee, 'access', _721 => _721.name, 'optionalAccess', _722 => _722.escapedText]) !== "slice") {
      return false;
    }
...
${this.getIden(level)}}()`;
  ...
  printSliceCall(node, identation, name = void 0, parsedArg = void 0, parsedArg2 = void 0) {
    parsedArg = this.goPrintCallArgument(_optionalChain([node, 'access', _723 => _723.arguments, 'optionalAccess', _724 => _724[0]]), parsedArg);
    parsedArg2 = this.goPrintCallArgument(_optionalChain([node, 'access', _725 => _725.arguments, 'optionalAccess', _726 => _726[1]]), parsedArg2);
  ...
  printReplaceCall(node, identation, name = void 0, parsedArg = void 0, parsedArg2 = void 0) {
    if (parsedArg !== void 0 && parsedArg2 !== void 0 && this.goNativeStringOperands([_optionalChain([node, 'access', _727 => _727.expression, 'optionalAccess', _728 => _728.expression]), _optionalChain([node, 'access', _729 => _729.arguments, 'optionalAccess', _730 => _730[0]]), _optionalChain([node, 'access', _731 => _731.arguments, 'optionalAccess', _732 => _732[1]])], [name, parsedArg, parsedArg2], ["string", "string", "string"])) {
  ...
  printReplaceAllCall(node, identation, name = void 0, parsedArg = void 0, parsedArg2 = void 0) {
    if (parsedArg !== void 0 && parsedArg2 !== void 0 && this.goNativeStringOperands([_optionalChain([node, 'access', _733 => _733.expression, 'optionalAccess', _734 => _734.expression]), _optionalChain([node, 'access', _735 => _735.arguments, 'optionalAccess', _736 => _736[0]]), _optionalChain([node, 'access', _737 => _737.arguments, 'optionalAccess', _738 => _738[1]])], [name, parsedArg, parsedArg2], ["string", "string", "string"])) {
  ...
  printThrowStatement(node, identation) {
    ...
    if (node.expression.kind === _typescript2.default.SyntaxKind.NewExpression) {
      const expression = node.expression;
      const argumentsExp = _nullishCoalesce(_optionalChain([expression, 'optionalAccess', _739 => _739.arguments]), () => ( []));
      const parsedArg = _nullishCoalesce(argumentsExp.map((n) => this.printNode(n, 0)).join(","), () => ( ""));
      const newExpression = this.printNode(expression.expression, 0);
      if (expression.expression.kind === _typescript2.default.SyntaxKind.Identifier) {
        const id = expression.expression;
        const symbol = this.getChecker().getSymbolAtLocation(expression.expression);
        if (symbol) {
          const declarations = _nullishCoalesce(_optionalChain([this, 'access', _740 => _740.getChecker, 'call', _741 => _741(), 'access', _742 => _742.getDeclaredTypeOfSymbol, 'call', _743 => _743(symbol), 'access', _744 => _744.symbol, 'optionalAccess', _745 => _745.declarations]), () => ( []));
          const isClassDeclaration = declarations.find((l) => l.kind === _typescript2.default.SyntaxKind.InterfaceDeclaration || l.kind === _typescript2.default.SyntaxKind.ClassDeclaration);
          if (isClassDeclaration) {
          } else {
...
${this.getIden(identation)}return nil`;
  ...
  printBinaryExpression(node, identation) {
    ...
    if (operatorToken.kind === _typescript2.default.SyntaxKind.EqualsToken) {
      ...
      if (_optionalChain([right, 'optionalAccess', _746 => _746.kind]) === _typescript2.default.SyntaxKind.AwaitExpression || rightSide.startsWith("<-this.callInternal")) {
        const leftParsed = this.printNode(left, 0);
        const awaited = _optionalChain([right, 'optionalAccess', _747 => _747.kind]) === _typescript2.default.SyntaxKind.AwaitExpression ? this.printNode(right, identation) : rightSide;
        return `
...
${tryBodyBlock}
  ...
  printNewExpression(node, identation) {
    let expression = _optionalChain([node, 'access', _748 => _748.expression, 'optionalAccess', _749 => _749.escapedText]);
    expression = expression ? expression : this.printNode(node.expression);
  ...
  goIsDerefStringKeyExpression(node) {
    return _optionalChain([node, 'optionalAccess', _750 => _750.kind]) === _typescript2.default.SyntaxKind.Identifier && this.goDeclaredTypeOfIdentifier(node) === "*string";
  }
...
${this.getIden(level)}}()`;
  ...
  isGoThisPropertyAccessExpression(node) {
    return _optionalChain([node, 'optionalAccess', _751 => _751.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression && _optionalChain([node, 'access', _752 => _752.expression, 'optionalAccess', _753 => _753.kind]) === _typescript2.default.SyntaxKind.ThisKeyword;
  }
  ...
  isGoElementAccessAssignmentTarget(node) {
    const parent = node.parent;
    return _optionalChain([parent, 'optionalAccess', _754 => _754.kind]) === _typescript2.default.SyntaxKind.BinaryExpression && parent.left === node && (parent.operatorToken.kind === _typescript2.default.SyntaxKind.EqualsToken || parent.operatorToken.kind === _typescript2.default.SyntaxKind.PlusEqualsToken);
  }
  ...
  goIntIndexExpression(node) {
    switch (_optionalChain([node, 'optionalAccess', _755 => _755.kind])) {
      case _typescript2.default.SyntaxKind.NumericLiteral: {
      ...
      case _typescript2.default.SyntaxKind.PrefixUnaryExpression:
        return node.operator === _typescript2.default.SyntaxKind.MinusToken && this.goIntIndexExpression(node.operand);
      case _typescript2.default.SyntaxKind.Identifier:
        return this.goIntOperandIdentifier(node);
      case _typescript2.default.SyntaxKind.BinaryExpression: {
        const op = _optionalChain([node, 'access', _756 => _756.operatorToken, 'optionalAccess', _757 => _757.kind]);
  ...
  goIntOperandIdentifier(node) {
    ...
    const declaration = _optionalChain([symbol, 'optionalAccess', _758 => _758.valueDeclaration]);
    if (_optionalChain([declaration, 'optionalAccess', _759 => _759.kind]) !== _typescript2.default.SyntaxKind.VariableDeclaration || declaration.initializer === void 0) {
      return false;
    }
    const list = declaration.parent;
    const loop = _optionalChain([list, 'optionalAccess', _760 => _760.parent]);
    if (_optionalChain([list, 'optionalAccess', _761 => _761.kind]) !== _typescript2.default.SyntaxKind.VariableDeclarationList || _optionalChain([list, 'access', _762 => _762.declarations, 'optionalAccess', _763 => _763.length]) !== 1 || _optionalChain([loop, 'optionalAccess', _764 => _764.kind]) !== _typescript2.default.SyntaxKind.ForStatement || loop.initializer !== list) {
      return false;
    }
  ...
  goSafeListUnboxIdentifier(node) {
    ...
    const declaration = _optionalChain([symbol, 'optionalAccess', _765 => _765.valueDeclaration]);
    return _optionalChain([declaration, 'optionalAccess', _766 => _766.kind]) === _typescript2.default.SyntaxKind.VariableDeclaration && this.goSafeListLocalUnbox(declaration) === GO_SAFE_LIST_LOCAL_TYPE;
  }
...
${this.getIden(level)}}()`;
  ...
  printElementAccessExpression(node, identation) {
    ...
    if (_optionalChain([baseExpr, 'optionalAccess', _767 => _767.kind]) === _typescript2.default.SyntaxKind.Identifier && this.goSafeListUnboxIdentifier(baseExpr)) {
...
var JavaTranspiler = class extends BaseTranspiler {
  ...
  countRequiredParameters(declaration) {
    const params = _nullishCoalesce(_optionalChain([declaration, 'optionalAccess', _768 => _768.parameters]), () => ( []));
    let required = 0;
  ...
  printArgsForCallExpression(node, identation) {
    ...
    const callee = node.expression;
    const isThisCall = _optionalChain([callee, 'optionalAccess', _769 => _769.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression && _optionalChain([callee, 'access', _770 => _770.expression, 'optionalAccess', _771 => _771.kind]) === _typescript2.default.SyntaxKind.ThisKeyword;
    if (isThisCall && args.length > 0) {
      ...
      if (isNullish) {
        const signature = this.getChecker().getResolvedSignature(node);
        const declaration = _optionalChain([signature, 'optionalAccess', _772 => _772.declaration]);
        if (declaration !== void 0) {
  ...
  printIdentifier(node) {
    ...
    const isInsideNewExpression = _optionalChain([node, 'optionalAccess', _773 => _773.parent, 'optionalAccess', _774 => _774.kind]) === _typescript2.default.SyntaxKind.NewExpression;
    const isInsideCatch = _optionalChain([node, 'optionalAccess', _775 => _775.parent, 'optionalAccess', _776 => _776.kind]) === _typescript2.default.SyntaxKind.ThrowStatement;
    const isLeftSide = _optionalChain([node, 'optionalAccess', _777 => _777.parent, 'optionalAccess', _778 => _778.name]) === node || _optionalChain([node, 'optionalAccess', _779 => _779.parent, 'optionalAccess', _780 => _780.left]) === node;
    const isCallOrPropertyAccess = _optionalChain([node, 'optionalAccess', _781 => _781.parent, 'optionalAccess', _782 => _782.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([node, 'optionalAccess', _783 => _783.parent, 'optionalAccess', _784 => _784.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression;
    if (!isLeftSide && !isCallOrPropertyAccess && !isInsideCatch && !isInsideNewExpression) {
      const type = this.getChecker().getTypeAtLocation(node);
      const typeSymbol = _optionalChain([type, 'optionalAccess', _785 => _785.symbol]);
      if (typeSymbol !== void 0) {
        const decl = _nullishCoalesce(_optionalChain([typeSymbol, 'optionalAccess', _786 => _786.declarations]), () => ( []));
        let isBuiltIn = void 0;
        if (decl.length > 0) {
          isBuiltIn = decl[0].getSourceFile().fileName.indexOf("typescript") > -1;
        }
        if (isBuiltIn !== void 0 && !isBuiltIn) {
          ...
          if (symbol) {
            ...
            if (first.kind === _typescript2.default.SyntaxKind.ImportSpecifier) {
              ...
              if (_optionalChain([importedSymbol, 'optionalAccess', _787 => _787.declarations, 'access', _788 => _788[0], 'optionalAccess', _789 => _789.kind]) === _typescript2.default.SyntaxKind.ClassDeclaration) {
                isClassDeclaration = true;
              }
            }
          }
  ...
  printConstructorDeclaration(node, identation) {
    ...
    let superCallParams = "";
    let hasSuperCall = false;
    _optionalChain([node, 'access', _790 => _790.body, 'optionalAccess', _791 => _791.statements, 'access', _792 => _792.forEach, 'call', _793 => _793((statement) => {
      if (_typescript2.default.isExpressionStatement(statement)) {
        const expression = statement.expression;
        if (_typescript2.default.isCallExpression(expression)) {
          const expressionText = expression.expression.getText().trim();
  ...
  printDynamicCall(node, identation) {
    const elementAccess = node.expression;
    if (_optionalChain([elementAccess, 'optionalAccess', _794 => _794.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression) {
      const parsedArg = _optionalChain([node, 'access', _795 => _795.arguments, 'optionalAccess', _796 => _796.length]) > 0 ? node.arguments.map((n) => this.printNode(n, identation).trimStart()).join(", ") : "";
      const target = this.printNode(elementAccess.expression, 0);
      const propName = this.printNode(elementAccess.argumentExpression, 0);
      const argsArray = `new Object[] { ${parsedArg} }`;
      const open = this.DYNAMIC_CALL_OPEN;
  ...
  getExpressionStatementPrefixesIfAny(node, identation) {
    const finalVars = [];
    if (_optionalChain([node, 'access', _797 => _797.expression, 'optionalAccess', _798 => _798.kind]) === _typescript2.default.SyntaxKind.CallExpression) {
  ...
  printWrappedUnknownThisProperty(node) {
    const type = this.getChecker().getResolvedSignature(node);
    if (_optionalChain([type, 'optionalAccess', _799 => _799.declaration]) === void 0) {
      let parsedArguments = _optionalChain([node, 'access', _800 => _800.arguments, 'optionalAccess', _801 => _801.map, 'call', _802 => _802((a) => this.printNode(a, 0)), 'access', _803 => _803.join, 'call', _804 => _804(", ")]);
      parsedArguments = parsedArguments ? parsedArguments : "";
      const propName = _optionalChain([node, 'access', _805 => _805.expression, 'optionalAccess', _806 => _806.name, 'access', _807 => _807.escapedText]);
      const isAsyncDecl = _optionalChain([node, 'optionalAccess', _808 => _808.parent, 'optionalAccess', _809 => _809.kind]) === _typescript2.default.SyntaxKind.AwaitExpression;
      const argsArray = `new Object[] { ${parsedArguments} }`;
      const open = this.DYNAMIC_CALL_OPEN;
  ...
  printOutOfOrderCallExpressionIfAny(node, identation) {
    ...
    if (node.expression.kind === _typescript2.default.SyntaxKind.PropertyAccessExpression) {
      ...
      const leftSide = _optionalChain([node, 'access', _810 => _810.expression, 'optionalAccess', _811 => _811.expression]);
      const leftSideText = leftSide ? this.printNode(leftSide, 0) : void 0;
      if (leftSideText === this.THIS_TOKEN || leftSide.getFullText().indexOf("(this as any)") > -1) {
        const res = this.printWrappedUnknownThisProperty(node);
        if (res)
  ...
  getVarMethodIfAny(node) {
    let current = _optionalChain([node, 'optionalAccess', _812 => _812.parent]);
    while (current) {
      if (_typescript2.default.isMethodDeclaration(current) || _typescript2.default.isFunctionDeclaration(current)) {
        return String(_nullishCoalesce(_optionalChain([current, 'access', _813 => _813.name, 'optionalAccess', _814 => _814.escapedText]), () => ( "")));
      }
      current = current.parent;
    }
  ...
  getVarClassIfAny(node) {
    let current = _optionalChain([node, 'optionalAccess', _815 => _815.parent]);
    while (current) {
      if (_typescript2.default.isClassDeclaration(current)) {
        return String(_nullishCoalesce(_optionalChain([current, 'access', _816 => _816.name, 'optionalAccess', _817 => _817.escapedText]), () => ( "")));
      }
      current = current.parent;
    }
    return "";
  ...
  getVarKey(node) {
    const varName = _nullishCoalesce(_optionalChain([node, 'optionalAccess', _818 => _818.escapedText]), () => ( _optionalChain([node, 'optionalAccess', _819 => _819.name, 'optionalAccess', _820 => _820.escapedText])));
    if (!varName) {
      return "";
    }
  ...
  isNullishLiteral(node) {
    return _optionalChain([node, 'optionalAccess', _821 => _821.kind]) === _typescript2.default.SyntaxKind.NullKeyword || _optionalChain([node, 'optionalAccess', _822 => _822.kind]) === _typescript2.default.SyntaxKind.Identifier && node.escapedText === "undefined";
  }
  ...
  javaNativeLengthKind(node) {
    if (_optionalChain([node, 'optionalAccess', _823 => _823.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || node.name.escapedText !== "length") {
      return void 0;
    }
  ...
  isJavaPrimitiveCounterReference(node) {
    ...
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _824 => _824.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _825 => _825.declarations, 'optionalAccess', _826 => _826[0]])));
  ...
  javaLocalNumberKind(node, depth) {
    ...
    const symbol = this.getChecker().getSymbolAtLocation(node);
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _832 => _832.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _833 => _833.declarations, 'optionalAccess', _834 => _834[0]])));
    if (!declaration || declaration.kind !== _typescript2.default.SyntaxKind.VariableDeclaration || !_typescript2.default.isIdentifier(declaration.name)) {
      return void 0;
    }
    ...
    const isConst = (_optionalChain([declaration, 'access', _835 => _835.parent, 'optionalAccess', _836 => _836.flags]) & _typescript2.default.NodeFlags.Const) !== 0;
  ...
  enclosingFunctionLike(node) {
    let current = _optionalChain([node, 'optionalAccess', _837 => _837.parent]);
    while (current) {
  ...
  printNativeEqualityIfProvable(node, leftText, rightText) {
    ...
    const checker = this.getChecker();
    const leftFamily = this.equalityOperandFamily(_optionalChain([checker, 'optionalAccess', _838 => _838.getTypeAtLocation, 'call', _839 => _839(node.left)]));
    const rightFamily = this.equalityOperandFamily(_optionalChain([checker, 'optionalAccess', _840 => _840.getTypeAtLocation, 'call', _841 => _841(node.right)]));
  ...
  isPlainHashMapReceiver(container, keys) {
    ...
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _842 => _842.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _843 => _843.declarations, 'optionalAccess', _844 => _844[0]])));
  ...
  callAlwaysReturnsPlainHashMap(node, depth) {
    ...
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _845 => _845.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _846 => _846.declarations, 'optionalAccess', _847 => _847[0]])));
  ...
  returnTypePrintsAsHashMap(declaration) {
    try {
      ...
      const type = _optionalChain([signature, 'optionalAccess', _848 => _848.getReturnType, 'call', _849 => _849()]);
      ...
      const declarations = _nullishCoalesce(_optionalChain([type, 'access', _850 => _850.getSymbol, 'call', _851 => _851(), 'optionalAccess', _852 => _852.declarations]), () => ( []));
  ...
  isVarargsArrayReference(node, depth = 0) {
    ...
    const symbol = this.getChecker().getSymbolAtLocation(node);
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _853 => _853.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _854 => _854.declarations, 'optionalAccess', _855 => _855[0]])));
    if (!declaration) {
      return false;
    }
  ...
  isJavaListType(type) {
    ...
    return _optionalChain([type, 'access', _856 => _856.target, 'optionalAccess', _857 => _857.symbol, 'optionalAccess', _858 => _858.escapedName]) === "ReadonlyArray";
  }
  ...
  isJavaListBackedClassType(type) {
    ...
    for (let depth = 0; current && depth < 8; depth++) {
      const name = _optionalChain([current, 'access', _859 => _859.symbol, 'optionalAccess', _860 => _860.escapedName]);
      if (name !== void 0 && JAVA_LIST_BACKED_TS_CLASSES.has(name)) {
        return true;
      }
      const bases = _nullishCoalesce(_optionalChain([current, 'access', _861 => _861.getBaseTypes, 'optionalCall', _862 => _862()]), () => ( []));
  ...
  isJavaPrimitiveForCounter(node) {
    ...
    const incrementor = forStatement.incrementor;
    if (!incrementor || _optionalChain([incrementor, 'access', _863 => _863.operand, 'optionalAccess', _864 => _864.kind]) !== _typescript2.default.SyntaxKind.Identifier || incrementor.operand.escapedText !== node.escapedText) {
      return false;
    }
  ...
  javaPrintedCallKind(node) {
    const callee = node.expression;
    if (_optionalChain([callee, 'optionalAccess', _865 => _865.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression) {
      return void 0;
    }
    const name = _optionalChain([callee, 'access', _866 => _866.name, 'optionalAccess', _867 => _867.escapedText]);
    const argCount = _nullishCoalesce(_optionalChain([node, 'access', _868 => _868.arguments, 'optionalAccess', _869 => _869.length]), () => ( 0));
    const onMath = _optionalChain([callee, 'access', _870 => _870.expression, 'optionalAccess', _871 => _871.kind]) === _typescript2.default.SyntaxKind.Identifier && callee.expression.escapedText === "Math";
    switch (name) {
      case "indexOf":
      case "search":
  ...
  isJavaMapType(type) {
    ...
    const declarations = _nullishCoalesce(_optionalChain([type, 'access', _872 => _872.getSymbol, 'call', _873 => _873(), 'optionalAccess', _874 => _874.declarations]), () => ( []));
  ...
  javaRepeatableOperand(node) {
    ...
    return _typescript2.default.isPropertyAccessExpression(node) && _optionalChain([node, 'access', _875 => _875.expression, 'optionalAccess', _876 => _876.kind]) === _typescript2.default.SyntaxKind.ThisKeyword;
  }
  ...
  javaDeclaredTypeOf(expression) {
    ...
    if (declaration === void 0 || expression.escapedText !== _optionalChain([declaration, 'access', _877 => _877.name, 'optionalAccess', _878 => _878.escapedText])) {
      return void 0;
    }
...
`;
  ...
  isJavaMapStructureType(type) {
    ...
    const declaration = _optionalChain([symbol, 'optionalAccess', _879 => _879.declarations, 'optionalAccess', _880 => _880[0]]);
    const fileName = _optionalChain([declaration, 'optionalAccess', _881 => _881.getSourceFile, 'optionalCall', _882 => _882(), 'optionalAccess', _883 => _883.fileName]);
  ...
  tupleRequiredElementCount(type) {
    const flags = _nullishCoalesce(_nullishCoalesce(_optionalChain([type, 'optionalAccess', _884 => _884.target, 'optionalAccess', _885 => _885.elementFlags]), () => ( _optionalChain([type, 'optionalAccess', _886 => _886.elementFlags]))), () => ( []));
    let required = 0;
  ...
  javaStringElementsReceiver(node) {
    ...
    const declaration = _optionalChain([this, 'access', _887 => _887.getChecker, 'call', _888 => _888(), 'access', _889 => _889.getSymbolAtLocation, 'call', _890 => _890(node), 'optionalAccess', _891 => _891.valueDeclaration]);
    ...
    if (_typescript2.default.isCallExpression(initializer) && _typescript2.default.isPropertyAccessExpression(initializer.expression)) {
      return _optionalChain([initializer, 'access', _892 => _892.expression, 'access', _893 => _893.name, 'optionalAccess', _894 => _894.escapedText]) === "split";
    }
    if (_typescript2.default.isArrayLiteralExpression(initializer)) {
  ...
  isLeftSideOfAssignment(node) {
    const parent = node.parent;
    if (_optionalChain([parent, 'optionalAccess', _895 => _895.kind]) !== _typescript2.default.SyntaxKind.BinaryExpression || parent.left !== node) {
      return false;
    }
  ...
  javaDeclarationOfIdentifier(expression) {
    ...
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _896 => _896.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _897 => _897.declarations, 'optionalAccess', _898 => _898[0]])));
    if (declaration === void 0) {
      return void 0;
    }
  ...
  javaDeclaredMapReceiver(expression) {
    ...
    if (expression.escapedText !== _optionalChain([declaration, 'access', _899 => _899.name, 'optionalAccess', _900 => _900.escapedText])) {
      return false;
    }
    let type;
    try {
  ...
  javaNativeConcat(node) {
    if (_optionalChain([node, 'optionalAccess', _901 => _901.operatorToken, 'optionalAccess', _902 => _902.kind]) !== _typescript2.default.SyntaxKind.PlusToken) {
      return false;
    }
  ...
  javaThisCallNumericKind(node) {
    if (_optionalChain([node, 'optionalAccess', _903 => _903.kind]) !== _typescript2.default.SyntaxKind.CallExpression) {
      return void 0;
    }
    const callee = node.expression;
    if (_optionalChain([callee, 'optionalAccess', _904 => _904.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([callee, 'access', _905 => _905.expression, 'optionalAccess', _906 => _906.kind]) !== _typescript2.default.SyntaxKind.ThisKeyword) {
      return void 0;
    }
    const name = _optionalChain([callee, 'access', _907 => _907.name, 'optionalAccess', _908 => _908.escapedText]);
    if (typeof name !== "string") {
      return void 0;
    }
    ...
    let declaration;
    try {
      declaration = _optionalChain([this, 'access', _909 => _909.getChecker, 'call', _910 => _910(), 'access', _911 => _911.getResolvedSignature, 'call', _912 => _912(node), 'optionalAccess', _913 => _913.declaration]);
    } catch (e) {
      declaration = void 0;
    }
    const fileName = _optionalChain([declaration, 'optionalAccess', _914 => _914.getSourceFile, 'optionalCall', _915 => _915(), 'access', _916 => _916.fileName]);
  ...
  javaIdentifierKeepsDeclaredName(node) {
    let declaration;
    try {
      declaration = _optionalChain([this, 'access', _917 => _917.getChecker, 'call', _918 => _918(), 'access', _919 => _919.getSymbolAtLocation, 'call', _920 => _920(node), 'optionalAccess', _921 => _921.valueDeclaration]);
    } catch (e) {
      return false;
    }
    ...
    return String(node.escapedText) === String(_optionalChain([declaration, 'access', _922 => _922.name, 'optionalAccess', _923 => _923.escapedText]));
  }
  ...
  javaNativeArithmeticKind(node, allowDeclaredLocals = true) {
    const op = _optionalChain([node, 'optionalAccess', _924 => _924.operatorToken, 'optionalAccess', _925 => _925.kind]);
  ...
  javaBaseTimeLongCall(node) {
    if (_optionalChain([node, 'optionalAccess', _926 => _926.kind]) !== _typescript2.default.SyntaxKind.CallExpression) {
      return false;
    }
    const callee = node.expression;
    if (_optionalChain([callee, 'optionalAccess', _927 => _927.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || callee.expression.kind !== _typescript2.default.SyntaxKind.ThisKeyword) {
      return false;
    }
    const name = _optionalChain([callee, 'access', _928 => _928.name, 'optionalAccess', _929 => _929.escapedText]);
    if (name !== "milliseconds" && name !== "seconds") {
      return false;
    }
    let declaration;
    try {
      declaration = _optionalChain([this, 'access', _930 => _930.getChecker, 'call', _931 => _931(), 'access', _932 => _932.getResolvedSignature, 'call', _933 => _933(node), 'optionalAccess', _934 => _934.declaration]);
    } catch (e) {
      declaration = void 0;
    }
    if (declaration === void 0) {
    ...
    const fileName = _nullishCoalesce(_optionalChain([declaration, 'access', _935 => _935.getSourceFile, 'optionalCall', _936 => _936(), 'optionalAccess', _937 => _937.fileName]), () => ( ""));
  ...
  javaIntForCounter(node) {
    if (_optionalChain([node, 'optionalAccess', _938 => _938.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    const symbol = this.getChecker().getSymbolAtLocation(node);
    const declaration = _optionalChain([symbol, 'optionalAccess', _939 => _939.valueDeclaration]);
    if (declaration === void 0 || !_typescript2.default.isVariableDeclaration(declaration)) {
      return false;
    }
    const declarationList = declaration.parent;
  ...
  javaLengthIntRead(node) {
    if (_optionalChain([node, 'optionalAccess', _940 => _940.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([node, 'access', _941 => _941.name, 'optionalAccess', _942 => _942.escapedText]) !== "length") {
      return false;
    }
  ...
  javaNativeSplitCall(node, name, parsedArg) {
    ...
    if (_optionalChain([callee, 'access', _943 => _943.name, 'optionalAccess', _944 => _944.escapedText]) !== "split" || _optionalChain([node, 'access', _945 => _945.arguments, 'optionalAccess', _946 => _946.length]) !== 1) {
      return void 0;
    }
  ...
  javaProvableCounterInt(node) {
    ...
    const symbol = this.getChecker().getSymbolAtLocation(node);
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _947 => _947.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _948 => _948.declarations, 'optionalAccess', _949 => _949[0]])));
    ...
    const list = declaration.parent;
    const forStatement = _optionalChain([list, 'optionalAccess', _950 => _950.parent]);
    ...
    const scan = (n) => {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.BinaryExpression && JAVA_ASSIGNMENT_OPERATOR_KINDS.has(n.operatorToken.kind) && _optionalChain([n, 'access', _953 => _953.left, 'optionalAccess', _954 => _954.kind]) === _typescript2.default.SyntaxKind.Identifier && n.left.escapedText === name) {
        safe = false;
        return;
      }
      if ((n.kind === _typescript2.default.SyntaxKind.PostfixUnaryExpression || n.kind === _typescript2.default.SyntaxKind.PrefixUnaryExpression) && _optionalChain([n, 'access', _955 => _955.operand, 'optionalAccess', _956 => _956.kind]) === _typescript2.default.SyntaxKind.Identifier && n.operand.escapedText === name) {
        return;
      }
    ...
    if (enclosing) {
      ...
      const scanOuter = (n) => {
        ...
        if (n.kind === _typescript2.default.SyntaxKind.BinaryExpression && JAVA_ASSIGNMENT_OPERATOR_KINDS.has(n.operatorToken.kind) && _optionalChain([n, 'access', _957 => _957.left, 'optionalAccess', _958 => _958.kind]) === _typescript2.default.SyntaxKind.Identifier && n.left.escapedText === name) {
          safe = false;
          return;
        }
  ...
  printNativeScalarParse(node, callee) {
    const args = _optionalChain([node, 'optionalAccess', _959 => _959.arguments]);
  ...
  printNativePadStart(node, name) {
    const args = _optionalChain([node, 'optionalAccess', _960 => _960.arguments]);
    ...
    const receiver = _optionalChain([node, 'access', _961 => _961.expression, 'optionalAccess', _962 => _962.expression]);
  ...
  javaUnwrapParentheses(node) {
    let current = node;
    while (_optionalChain([current, 'optionalAccess', _963 => _963.kind]) === _typescript2.default.SyntaxKind.ParenthesizedExpression) {
      current = current.expression;
    }
    return current;
  }
  ...
  javaNativeArithmeticType(node) {
    ...
    if (_optionalChain([value, 'optionalAccess', _964 => _964.kind]) !== _typescript2.default.SyntaxKind.BinaryExpression) {
      return void 0;
    }
  ...
  javaEnclosingFunction(node) {
    let current = _optionalChain([node, 'optionalAccess', _965 => _965.parent]);
    while (current !== void 0) {
  ...
  javaScopingBlock(node) {
    let current = _optionalChain([node, 'optionalAccess', _966 => _966.parent]);
    let last = void 0;
    while (current !== void 0) {
  ...
  javaArithmeticLocalUseIsSafe(node, declaration, javaType) {
    ...
    if (parent.kind === _typescript2.default.SyntaxKind.AsExpression || parent.kind === _typescript2.default.SyntaxKind.TypeAssertionExpression) {
      return javaType === "String" && _optionalChain([parent, 'access', _967 => _967.type, 'optionalAccess', _968 => _968.kind]) === _typescript2.default.SyntaxKind.StringKeyword;
    }
    ...
    if (parent.kind === _typescript2.default.SyntaxKind.ElementAccessExpression && parent.expression === node) {
      ...
      if (_optionalChain([grand, 'optionalAccess', _969 => _969.kind]) === _typescript2.default.SyntaxKind.DeleteExpression) {
        return false;
      }
      if (_optionalChain([grand, 'optionalAccess', _970 => _970.kind]) === _typescript2.default.SyntaxKind.BinaryExpression && grand.left === parent && JAVA_ASSIGNMENT_OPERATOR_KINDS.has(grand.operatorToken.kind)) {
        return false;
      }
    }
    if (parent.kind === _typescript2.default.SyntaxKind.ArrayLiteralExpression) {
      const grand = parent.parent;
      if (_optionalChain([grand, 'optionalAccess', _971 => _971.kind]) === _typescript2.default.SyntaxKind.BinaryExpression && grand.left === parent && JAVA_ASSIGNMENT_OPERATOR_KINDS.has(grand.operatorToken.kind)) {
        return false;
      }
    }
  ...
  getObjectLiteralFromCallExpressionArguments(node) {
    const res = [];
    if (!_optionalChain([node, 'optionalAccess', _972 => _972.arguments])) {
      return res;
    }
    const args = node.arguments;
  ...
  getBinaryExpressionPrefixes(node, identation) {
    let right = _optionalChain([node, 'optionalAccess', _973 => _973.right]);
    if (_optionalChain([right, 'optionalAccess', _974 => _974.kind]) === _typescript2.default.SyntaxKind.AwaitExpression) {
  ...
  analyzeFinalVars(fnBody) {
    ...
    const symbolIdOf = (n) => {
      try {
        const checker = this.getChecker();
        const sym = _optionalChain([checker, 'optionalAccess', _975 => _975.getSymbolAtLocation, 'optionalCall', _976 => _976(n)]);
        const decl = _nullishCoalesce(_optionalChain([sym, 'optionalAccess', _977 => _977.declarations, 'optionalAccess', _978 => _978[0]]), () => ( _optionalChain([sym, 'optionalAccess', _979 => _979.valueDeclaration])));
        if (decl)
    ...
    const discoverPotentialReassignments = (node) => {
      if (!node)
        return;
      if (node.kind === _typescript2.default.SyntaxKind.BinaryExpression) {
        if (_optionalChain([node, 'access', _980 => _980.left, 'optionalAccess', _981 => _981.kind]) === _typescript2.default.SyntaxKind.Identifier) {
          reassignedSyms.add(symbolIdOf(node.left));
        } else if (node.operatorToken.kind === _typescript2.default.SyntaxKind.EqualsToken && _optionalChain([node, 'access', _982 => _982.left, 'optionalAccess', _983 => _983.kind]) === _typescript2.default.SyntaxKind.ArrayLiteralExpression) {
          for (const elem of _nullishCoalesce(node.left.elements, () => ( []))) {
            if (_optionalChain([elem, 'optionalAccess', _984 => _984.kind]) === _typescript2.default.SyntaxKind.Identifier) {
              reassignedSyms.add(symbolIdOf(elem));
            }
          }
        }
      }
      if ((node.kind === _typescript2.default.SyntaxKind.PrefixUnaryExpression || node.kind === _typescript2.default.SyntaxKind.PostfixUnaryExpression) && this.isIncDecOperator(node.operator) && _optionalChain([node, 'access', _985 => _985.operand, 'optionalAccess', _986 => _986.kind]) === _typescript2.default.SyntaxKind.Identifier) {
        reassignedSyms.add(symbolIdOf(node.operand));
      }
      if (node.kind === _typescript2.default.SyntaxKind.Identifier) {
        const name = node.escapedText;
        if (name && name !== "undefined" && !_optionalChain([name, 'access', _987 => _987.startsWith, 'optionalCall', _988 => _988("null")])) {
    ...
    const visitExprInObjLit = (n) => {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.Identifier) {
        ...
        if (name && name !== "undefined" && !_optionalChain([name, 'access', _989 => _989.startsWith, 'optionalCall', _990 => _990("null")])) {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.CallExpression) {
        _optionalChain([n, 'access', _991 => _991.arguments, 'optionalAccess', _992 => _992.forEach, 'call', _993 => _993(visitExprInObjLit)]);
        if (_optionalChain([n, 'access', _994 => _994.expression, 'optionalAccess', _995 => _995.kind]) === _typescript2.default.SyntaxKind.PropertyAccessExpression) {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.ElementAccessExpression) {
        ...
        while (_optionalChain([left, 'optionalAccess', _996 => _996.kind]) === _typescript2.default.SyntaxKind.ElementAccessExpression) {
      ...
      if (n.kind === _typescript2.default.SyntaxKind.ObjectLiteralExpression) {
        _optionalChain([n, 'access', _997 => _997.properties, 'optionalAccess', _998 => _998.forEach, 'call', _999 => _999((p) => {
          if (p.initializer)
    ...
    const walk = (node) => {
      ...
      if (node.kind === _typescript2.default.SyntaxKind.BinaryExpression && this.isAssignmentOperator(node.operatorToken.kind)) {
        walk(node.right);
        if (_optionalChain([node, 'access', _1000 => _1000.left, 'optionalAccess', _1001 => _1001.kind]) === _typescript2.default.SyntaxKind.Identifier) {
      ...
      if ((node.kind === _typescript2.default.SyntaxKind.PrefixUnaryExpression || node.kind === _typescript2.default.SyntaxKind.PostfixUnaryExpression) && this.isIncDecOperator(node.operator) && _optionalChain([node, 'access', _1002 => _1002.operand, 'optionalAccess', _1003 => _1003.kind]) === _typescript2.default.SyntaxKind.Identifier) {
      ...
      if (node.kind === _typescript2.default.SyntaxKind.ObjectLiteralExpression) {
        _optionalChain([node, 'access', _1004 => _1004.properties, 'optionalAccess', _1005 => _1005.forEach, 'call', _1006 => _1006((prop) => {
          if (prop.initializer) {
      ...
      if (node.kind === _typescript2.default.SyntaxKind.TryStatement) {
        ...
        walkBlockAndBump(_optionalChain([node, 'access', _1007 => _1007.catchClause, 'optionalAccess', _1008 => _1008.block]));
  ...
  getObjectLiteralId(node) {
    ...
    const fileName = _nullishCoalesce(_optionalChain([node, 'access', _1009 => _1009.getSourceFile, 'optionalCall', _1010 => _1010(), 'optionalAccess', _1011 => _1011.fileName]), () => ( ""));
  ...
  printVariableDeclarationList(node, identation) {
    ...
    if (this.removeVariableDeclarationForFunctionExpression && _optionalChain([declaration, 'optionalAccess', _1012 => _1012.initializer]) && _typescript2.default.isFunctionExpression(declaration.initializer)) {
      return this.printNode(declaration.initializer, identation).trimEnd();
    }
    if (_optionalChain([declaration, 'optionalAccess', _1013 => _1013.name, 'access', _1014 => _1014.kind]) === _typescript2.default.SyntaxKind.ArrayBindingPattern) {
      const arrayBindingPattern = declaration.name;
      const arrayBindingPatternElements = arrayBindingPattern.elements;
...
`;
    ...
    const isNew = _optionalChain([declaration, 'optionalAccess', _1015 => _1015.initializer]) && declaration.initializer.kind === _typescript2.default.SyntaxKind.NewExpression;
  ...
  printThisKeyword(node, identation) {
    let current = _optionalChain([node, 'optionalAccess', _1016 => _1016.parent]);
    while (current) {
      ...
      current = _optionalChain([current, 'optionalAccess', _1017 => _1017.parent]);
    }
    return this.THIS_TOKEN;
  }
  transformPropertyAcessExpressionIfNeeded(node) {
  ...
  printCustomDefaultValueIfNeeded(node) {
    ...
    if (_optionalChain([node, 'optionalAccess', _1018 => _1018.escapedText]) === "undefined" && _optionalChain([this, 'access', _1019 => _1019.getChecker, 'call', _1020 => _1020(), 'access', _1021 => _1021.getTypeAtLocation, 'call', _1022 => _1022(_optionalChain([node, 'optionalAccess', _1023 => _1023.parent])), 'optionalAccess', _1024 => _1024.flags]) === _typescript2.default.TypeFlags.Number) {
      return this.UNDEFINED_TOKEN;
    }
    return void 0;
  }
  ...
  isPureInitializer(node) {
    switch (_optionalChain([node, 'optionalAccess', _1025 => _1025.kind])) {
      case _typescript2.default.SyntaxKind.NullKeyword:
      case _typescript2.default.SyntaxKind.TrueKeyword:
      case _typescript2.default.SyntaxKind.FalseKeyword:
      case _typescript2.default.SyntaxKind.StringLiteral:
...
`;
  ...
  printFinalOutsideMethodVariableWrappersIfAny(node, identation) {
    const parameters = _optionalChain([node, 'optionalAccess', _1026 => _1026.parameters]);
  ...
  printInsideMethodVariableWrappersIfAny(node, identation) {
    const parameters = _optionalChain([node, 'optionalAccess', _1027 => _1027.parameters]);
    const finalVarWrappers = [];
    if (parameters) {
      const isAsyncMethod = this.isAsyncFunction(node);
      parameters.forEach((param) => {
  ...
  printNativeArrayIsArray(node, parsedArg) {
    const operand = _optionalChain([node, 'optionalAccess', _1028 => _1028.arguments, 'optionalAccess', _1029 => _1029[0]]);
  ...
  printNativeObjectKeysCall(node) {
    const argument = _optionalChain([node, 'optionalAccess', _1030 => _1030.arguments, 'optionalAccess', _1031 => _1031[0]]);
  ...
  printNativePromiseAllCall(node) {
    const awaitNode = _optionalChain([node, 'optionalAccess', _1032 => _1032.parent]);
    if (_optionalChain([awaitNode, 'optionalAccess', _1033 => _1033.kind]) !== _typescript2.default.SyntaxKind.AwaitExpression) {
      return void 0;
    }
    const listNode = _optionalChain([node, 'access', _1034 => _1034.arguments, 'optionalAccess', _1035 => _1035[0]]);
    if (_optionalChain([listNode, 'optionalAccess', _1036 => _1036.kind]) !== _typescript2.default.SyntaxKind.ArrayLiteralExpression) {
      return void 0;
    }
    ...
    if (_optionalChain([awaitNode, 'access', _1037 => _1037.parent, 'optionalAccess', _1038 => _1038.kind]) === _typescript2.default.SyntaxKind.ExpressionStatement) {
      return allOf;
    }
  ...
  isConstBoundIdentifier(node) {
    ...
    const declaration = _optionalChain([this, 'access', _1039 => _1039.getChecker, 'call', _1040 => _1040(), 'access', _1041 => _1041.getSymbolAtLocation, 'call', _1042 => _1042(node), 'optionalAccess', _1043 => _1043.valueDeclaration]);
    return _optionalChain([declaration, 'optionalAccess', _1044 => _1044.kind]) === _typescript2.default.SyntaxKind.VariableDeclaration && (_typescript2.default.getCombinedNodeFlags(declaration) & _typescript2.default.NodeFlags.Const) === _typescript2.default.NodeFlags.Const;
  }
  printPromiseAllCall(node, identation, parsedArg = void 0) {
  ...
  printMathFloorCall(node, _identation, parsedArg = void 0) {
    return `(Math.floor(Double.parseDouble(${this.javaStringBoxText(_optionalChain([node, 'optionalAccess', _1045 => _1045.arguments, 'optionalAccess', _1046 => _1046[0]]), parsedArg)})))`;
  }
  printMathRoundCall(node, _identation, parsedArg = void 0) {
    return `Math.round(Double.parseDouble(${this.javaStringBoxText(_optionalChain([node, 'optionalAccess', _1047 => _1047.arguments, 'optionalAccess', _1048 => _1048[0]]), parsedArg)}))`;
  }
  printMathCeilCall(node, _identation, parsedArg = void 0) {
    return `Math.ceil(Double.parseDouble(${this.javaStringBoxText(_optionalChain([node, 'optionalAccess', _1049 => _1049.arguments, 'optionalAccess', _1050 => _1050[0]]), parsedArg)}))`;
  }
  ...
  javaNativeIndexOfCall(node, name, parsedArg) {
    ...
    const receiver = _optionalChain([node, 'optionalAccess', _1051 => _1051.expression, 'optionalAccess', _1052 => _1052.expression]);
    if (receiver === void 0) {
      return void 0;
    }
    const checker = this.getChecker();
    ...
    const arg = _optionalChain([node, 'access', _1053 => _1053.arguments, 'optionalAccess', _1054 => _1054[0]]);
    if (arg === void 0) {
      return void 0;
    }
  ...
  nativeSliceCallIfProvable(node, name) {
    const args = _nullishCoalesce(_optionalChain([node, 'optionalAccess', _1055 => _1055.arguments]), () => ( []));
    if (args.length < 1 || args.length > 2) {
      return void 0;
    }
    ...
    const receiverExpression = _typescript2.default.isPropertyAccessExpression(_optionalChain([node, 'optionalAccess', _1056 => _1056.expression])) ? node.expression.expression : void 0;
  ...
  printReplaceAllCall(node, identation, name = void 0, parsedArg = void 0, parsedArg2 = void 0) {
    const pattern = this.stringLiteralArgument(_optionalChain([node, 'optionalAccess', _1057 => _1057.arguments, 'optionalAccess', _1058 => _1058[0]]));
    const replacement = this.stringLiteralArgument(_optionalChain([node, 'optionalAccess', _1059 => _1059.arguments, 'optionalAccess', _1060 => _1060[1]]));
    const receiver = this.sideEffectFreeReceiver(_optionalChain([node, 'optionalAccess', _1061 => _1061.expression]));
  ...
  javaPrimitiveCounter(node) {
    if (_optionalChain([node, 'optionalAccess', _1062 => _1062.kind]) !== _typescript2.default.SyntaxKind.Identifier) {
      return false;
    }
    const symbol = this.getChecker().getSymbolAtLocation(node);
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _1063 => _1063.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _1064 => _1064.declarations, 'optionalAccess', _1065 => _1065[0]])));
    ...
    const list = declaration.parent;
    if (_optionalChain([list, 'optionalAccess', _1066 => _1066.kind]) !== _typescript2.default.SyntaxKind.VariableDeclarationList || list.declarations.length !== 1) {
      return false;
    }
    const forStatement = list.parent;
    if (_optionalChain([forStatement, 'optionalAccess', _1067 => _1067.kind]) !== _typescript2.default.SyntaxKind.ForStatement || forStatement.initializer !== list) {
      return false;
    }
    if (this.javaIntegerLiteralKind(declaration.initializer) === void 0) {
      return false;
    ...
    const incrementor = forStatement.incrementor;
    return _optionalChain([incrementor, 'optionalAccess', _1068 => _1068.kind]) === _typescript2.default.SyntaxKind.PostfixUnaryExpression && _optionalChain([incrementor, 'access', _1069 => _1069.operand, 'optionalAccess', _1070 => _1070.kind]) === _typescript2.default.SyntaxKind.Identifier && incrementor.operand.escapedText === node.escapedText;
  }
  ...
  javaPreciseBooleanCall(node) {
    ...
    const declaration = _optionalChain([this, 'access', _1071 => _1071.getChecker, 'call', _1072 => _1072(), 'access', _1073 => _1073.getResolvedSignature, 'call', _1074 => _1074(node), 'optionalAccess', _1075 => _1075.declaration]);
    if (declaration === void 0 || declaration.kind !== _typescript2.default.SyntaxKind.MethodDeclaration || _optionalChain([declaration, 'access', _1076 => _1076.parent, 'optionalAccess', _1077 => _1077.kind]) !== _typescript2.default.SyntaxKind.ClassDeclaration || _optionalChain([declaration, 'access', _1078 => _1078.parent, 'access', _1079 => _1079.name, 'optionalAccess', _1080 => _1080.escapedText]) !== "Precise" || !_optionalChain([declaration, 'access', _1081 => _1081.modifiers, 'optionalAccess', _1082 => _1082.some, 'call', _1083 => _1083((modifier) => modifier.kind === _typescript2.default.SyntaxKind.StaticKeyword)])) {
      return false;
    }
  ...
  javaBooleanValueKind(node) {
    const type = this.getChecker().getTypeAtLocation(node);
    const flags = _nullishCoalesce(_optionalChain([type, 'optionalAccess', _1084 => _1084.flags]), () => ( 0));
  ...
  javaCalleeResolves(node) {
    ...
    return _optionalChain([signature, 'optionalAccess', _1085 => _1085.declaration]) !== void 0;
  }
  ...
  javaCallBooleanKind(node) {
    if (_optionalChain([node, 'optionalAccess', _1086 => _1086.kind]) !== _typescript2.default.SyntaxKind.CallExpression) {
      return void 0;
    }
    const callee = node.expression;
    if (_optionalChain([callee, 'optionalAccess', _1087 => _1087.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([callee, 'access', _1088 => _1088.expression, 'optionalAccess', _1089 => _1089.kind]) !== _typescript2.default.SyntaxKind.ThisKeyword) {
      return void 0;
    }
    ...
    const name = _optionalChain([callee, 'access', _1090 => _1090.name, 'optionalAccess', _1091 => _1091.escapedText]);
    ...
    const defaultArgument = _optionalChain([node, 'access', _1092 => _1092.arguments, 'optionalAccess', _1093 => _1093[defaultArgumentIndex]]);
    ...
    const defaultIsBoolean = defaultIsNullish || _optionalChain([defaultArgument, 'optionalAccess', _1094 => _1094.kind]) === _typescript2.default.SyntaxKind.TrueKeyword || _optionalChain([defaultArgument, 'optionalAccess', _1095 => _1095.kind]) === _typescript2.default.SyntaxKind.FalseKeyword;
  ...
  javaPrintsBooleanValue(node, seen) {
    ...
    switch (node.kind) {
      ...
      case _typescript2.default.SyntaxKind.PrefixUnaryExpression:
        return node.operator === _typescript2.default.SyntaxKind.ExclamationToken && this.javaPrintsBooleanValue(node.operand, seen);
      case _typescript2.default.SyntaxKind.BinaryExpression:
  ...
  javaBooleanBaseField(node) {
    ...
    let declaration;
    try {
      declaration = _optionalChain([this, 'access', _1098 => _1098.getChecker, 'call', _1099 => _1099(), 'access', _1100 => _1100.getSymbolAtLocation, 'call', _1101 => _1101(node.name), 'optionalAccess', _1102 => _1102.valueDeclaration]);
    } catch (e) {
      declaration = void 0;
    }
  ...
  javaBooleanBoxIdentifier(node, seen) {
    if (_optionalChain([node, 'optionalAccess', _1103 => _1103.kind]) !== _typescript2.default.SyntaxKind.Identifier || seen.has(node)) {
      return void 0;
    }
    let symbol;
    try {
    ...
    const decl = _optionalChain([symbol, 'optionalAccess', _1104 => _1104.valueDeclaration]);
    if (decl === void 0) {
      return void 0;
    }
  ...
  printThrowStatement(node, identation) {
    ...
    if (node.expression.kind === _typescript2.default.SyntaxKind.NewExpression) {
      const expression = node.expression;
      const argumentsExp = _nullishCoalesce(_optionalChain([expression, 'optionalAccess', _1105 => _1105.arguments]), () => ( []));
      const parsedArg = _nullishCoalesce(argumentsExp.map((n) => this.printNode(n, 0)).join(","), () => ( ""));
      const newExpression = this.printNode(expression.expression, 0);
      if (expression.expression.kind === _typescript2.default.SyntaxKind.Identifier) {
        const id = expression.expression;
        ...
        const symbol = this.getChecker().getSymbolAtLocation(expression.expression);
        if (symbol) {
          const declarations = _nullishCoalesce(_optionalChain([this, 'access', _1106 => _1106.getChecker, 'call', _1107 => _1107(), 'access', _1108 => _1108.getDeclaredTypeOfSymbol, 'call', _1109 => _1109(symbol), 'access', _1110 => _1110.symbol, 'optionalAccess', _1111 => _1111.declarations]), () => ( []));
...
var _RustTranspiler = class _RustTranspiler extends BaseTranspiler {
  ...
  rustReadPrintsValue(node) {
    ...
    const symbol = this.getChecker().getSymbolAtLocation(node);
    const declarations = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _1112 => _1112.declarations]), () => ( []));
    if (declarations.length === 0) {
      return false;
    }
    return declarations.every((declaration) => _typescript2.default.isParameter(declaration) || _typescript2.default.isVariableDeclaration(declaration) && _optionalChain([declaration, 'access', _1113 => _1113.initializer, 'optionalAccess', _1114 => _1114.kind]) !== SyntaxKind4.NewExpression);
  }
  ...
  printNativeStringIndexOf(node, receiverText) {
    if (node === void 0 || !_typescript2.default.isCallExpression(node) || !_typescript2.default.isPropertyAccessExpression(node.expression) || _optionalChain([node, 'access', _1115 => _1115.arguments, 'optionalAccess', _1116 => _1116.length]) !== 1) {
      return void 0;
    }
  ...
  isDeclaredValueIdentifier(node) {
    const symbol = this.getChecker().getSymbolAtLocation(node);
    const declarations = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _1117 => _1117.declarations]), () => ( []));
    if (declarations.length === 0) {
      return false;
    }
  ...
  rustNativeInsertReceiver(expr) {
    ...
    if (_typescript2.default.isPropertyAccessExpression(expr) && expr.expression.kind === SyntaxKind4.ThisKeyword && _optionalChain([expr, 'access', _1118 => _1118.name, 'optionalAccess', _1119 => _1119.kind]) === SyntaxKind4.Identifier) {
  ...
  rustReceiverStaysDict(baseExpr, receiver) {
    ...
    let declarations;
    try {
      declarations = _nullishCoalesce(_optionalChain([this, 'access', _1120 => _1120.getChecker, 'call', _1121 => _1121(), 'access', _1122 => _1122.getSymbolAtLocation, 'call', _1123 => _1123(ident), 'optionalAccess', _1124 => _1124.declarations]), () => ( []));
    } catch (e) {
      return false;
    }
    if (declarations.length !== 1) {
    ...
    const initializer = declaration.initializer;
    if (initializer === void 0 || _typescript2.default.isElementAccessExpression(initializer) || _optionalChain([declaration, 'access', _1125 => _1125.name, 'optionalAccess', _1126 => _1126.kind]) !== SyntaxKind4.Identifier) {
      return false;
    }
  ...
  printVariableDeclarationList(node, identation) {
    ...
    if (_optionalChain([declaration, 'optionalAccess', _1130 => _1130.name, 'access', _1131 => _1131.kind]) === SyntaxKind4.ArrayBindingPattern) {
...
`;
  ...
  rustCallPrintsBool(node) {
    if (_optionalChain([node, 'optionalAccess', _1132 => _1132.kind]) !== SyntaxKind4.CallExpression) {
      return false;
    }
    const callee = node.expression;
    let name;
    if (_optionalChain([callee, 'optionalAccess', _1133 => _1133.kind]) === SyntaxKind4.Identifier) {
      name = callee.escapedText;
    } else if (_optionalChain([callee, 'optionalAccess', _1134 => _1134.kind]) === SyntaxKind4.PropertyAccessExpression) {
      name = _optionalChain([callee, 'access', _1135 => _1135.name, 'optionalAccess', _1136 => _1136.escapedText]);
    }
  ...
  rustEnclosingFunction(node) {
    let current = _optionalChain([node, 'optionalAccess', _1138 => _1138.parent]);
    while (current) {
      switch (current.kind) {
  ...
  rustBindsName(node, name) {
    switch (_optionalChain([node, 'optionalAccess', _1139 => _1139.kind])) {
      ...
      case SyntaxKind4.FunctionExpression:
      case SyntaxKind4.ArrowFunction:
        return _optionalChain([node, 'access', _1140 => _1140.name, 'optionalAccess', _1141 => _1141.kind]) === SyntaxKind4.Identifier && node.name.escapedText === name;
    }
    return false;
  }
  ...
  rustSafeStringLocalInitializer(declaration) {
    const initializer = declaration.initializer;
    if (_optionalChain([declaration, 'access', _1142 => _1142.name, 'optionalAccess', _1143 => _1143.kind]) !== SyntaxKind4.Identifier || _optionalChain([initializer, 'optionalAccess', _1144 => _1144.kind]) !== SyntaxKind4.CallExpression) {
      return false;
    }
    const callee = initializer.expression;
    if (_optionalChain([callee, 'optionalAccess', _1145 => _1145.kind]) === SyntaxKind4.PropertyAccessExpression) {
      return _optionalChain([callee, 'access', _1146 => _1146.expression, 'optionalAccess', _1147 => _1147.kind]) === SyntaxKind4.ThisKeyword && _RustTranspiler.RUST_STRING_LOCAL_HELPERS.has(callee.name.escapedText);
    }
    if (_optionalChain([callee, 'optionalAccess', _1148 => _1148.kind]) === SyntaxKind4.Identifier) {
  ...
  rustStringLocalIdentifierIsTyped(node) {
    if (_optionalChain([node, 'optionalAccess', _1149 => _1149.kind]) !== SyntaxKind4.Identifier) {
      return false;
    }
    let symbol;
    try {
    ...
    const declaration = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _1150 => _1150.valueDeclaration]), () => ( _optionalChain([symbol, 'optionalAccess', _1151 => _1151.declarations, 'optionalAccess', _1152 => _1152[0]])));
    if (_optionalChain([declaration, 'optionalAccess', _1153 => _1153.kind]) !== SyntaxKind4.VariableDeclaration) {
      return false;
    }
  ...
  getRustBoolLocalInitializer(declaration, printedValue) {
    const initializer = declaration.initializer;
    if (initializer === void 0 || _optionalChain([declaration, 'access', _1154 => _1154.name, 'optionalAccess', _1155 => _1155.kind]) !== SyntaxKind4.Identifier) {
      return void 0;
    }
...
${classMethods}
  ...
  printFunctionDefinition(node, identation) {
    const name = _nullishCoalesce(_optionalChain([node, 'access', _1156 => _1156.name, 'optionalAccess', _1157 => _1157.escapedText]), () => ( ""));
    const params = node.parameters;
  ...
  printErrorConstructorArg(name, index, node, identation) {
    const kind = _optionalChain([_RustTranspiler, 'access', _1158 => _1158.RUST_ERROR_CONSTRUCTOR_ARGS, 'access', _1159 => _1159[name], 'optionalAccess', _1160 => _1160[index]]);
    if (kind === void 0) {
      return this.printNode(node, identation);
    }
  ...
  printNewExpression(node, identation) {
    let expression = _optionalChain([node, 'access', _1161 => _1161.expression, 'optionalAccess', _1162 => _1162.escapedText]);
    expression = expression ? expression : this.printNode(node.expression);
    if (expression === "Error") {
  ...
  typeSymbolOf(type) {
    if (type === void 0 || type === null)
      return void 0;
    return _nullishCoalesce(_nullishCoalesce(_optionalChain([type, 'access', _1163 => _1163.getSymbol, 'optionalCall', _1164 => _1164()]), () => ( type.symbol)), () => ( type.aliasSymbol));
  }
  ...
  isLibDeclaredType(type) {
    const declarations = _nullishCoalesce(_optionalChain([this, 'access', _1165 => _1165.typeSymbolOf, 'call', _1166 => _1166(type), 'optionalAccess', _1167 => _1167.declarations]), () => ( []));
    return declarations.some((d) => {
      const file = _nullishCoalesce(_optionalChain([d, 'optionalAccess', _1168 => _1168.getSourceFile, 'optionalCall', _1169 => _1169(), 'optionalAccess', _1170 => _1170.fileName]), () => ( ""));
  ...
  isClassInstanceType(type) {
    ...
    if (_optionalChain([symbol, 'optionalAccess', _1171 => _1171.flags]) & _typescript2.default.SymbolFlags.Class)
      return true;
    const declarations = _nullishCoalesce(_optionalChain([symbol, 'optionalAccess', _1172 => _1172.declarations]), () => ( []));
  ...
  isProvenListType(type) {
    if (!(type.flags & _typescript2.default.TypeFlags.Object))
      return false;
    const objectFlags = (_nullishCoalesce(type.objectFlags, () => ( 0))) | (_nullishCoalesce(_optionalChain([type, 'access', _1173 => _1173.target, 'optionalAccess', _1174 => _1174.objectFlags]), () => ( 0)));
    if (objectFlags & _typescript2.default.ObjectFlags.Tuple)
      return true;
    const name = _optionalChain([this, 'access', _1175 => _1175.typeSymbolOf, 'call', _1176 => _1176(type), 'optionalAccess', _1177 => _1177.getName, 'optionalCall', _1178 => _1178()]);
    if (name === "Array" || name === "ReadonlyArray")
      return true;
    const targetName = _optionalChain([this, 'access', _1179 => _1179.typeSymbolOf, 'call', _1180 => _1180(type.target), 'optionalAccess', _1181 => _1181.getName, 'optionalCall', _1182 => _1182()]);
  ...
  rustDeclarationOfIdentifier(node) {
    ...
    try {
      const symbol = this.getChecker().getSymbolAtLocation(node);
      return _optionalChain([symbol, 'optionalAccess', _1183 => _1183.valueDeclaration]);
    } catch (e) {
      return void 0;
    }
  }
  ...
  rustIsDeclaredDictLocal(node) {
    ...
    if (declaration === void 0)
      return false;
    const name = _optionalChain([declaration, 'access', _1184 => _1184.name, 'optionalAccess', _1185 => _1185.text]);
    if (typeof name !== "string")
      return false;
    let initializer;
...
${idn}}`;
  ...
  printIfStatement(node, identation) {
    ...
    const elseStatement = node.elseStatement;
    if (_optionalChain([elseStatement, 'optionalAccess', _1186 => _1186.kind]) === SyntaxKind4.Block) {
      ifComplete += ` else${this.printBlock(elseStatement, identation)}`;
    } else if (_optionalChain([elseStatement, 'optionalAccess', _1187 => _1187.kind]) === SyntaxKind4.IfStatement) {
...
${this.getIden(identation)}})`;
  ...
  printArrayIsArrayCall(node, identation, parsedArg = void 0) {
    const native = this.nativeValuePredicateText("array", _optionalChain([node, 'optionalAccess', _1188 => _1188.arguments, 'optionalAccess', _1189 => _1189[0]]), parsedArg);
    if (native !== void 0) {
  ...
  printIncludesCall(node, identation, name = void 0, parsedArg = void 0) {
    const pRef = _optionalChain([parsedArg, 'optionalAccess', _1190 => _1190.startsWith, 'call', _1191 => _1191("Value::")]) ? `&${parsedArg}` : `&${parsedArg}`;
  ...
  printTryStatement(node, identation) {
    ...
    const catchBody = node.catchClause.block.statements.map((s) => this.printNode(s, identation + 1)).join("\n");
    const rawName = _optionalChain([node, 'access', _1192 => _1192.catchClause, 'optionalAccess', _1193 => _1193.variableDeclaration, 'optionalAccess', _1194 => _1194.name, 'optionalAccess', _1195 => _1195.escapedText]);
...
var CppTranspiler = class extends BaseTranspiler {
  ...
  printConstructorDeclaration(node, identation) {
    ...
    let superCallParams = "";
    let hasSuperCall = false;
    _optionalChain([node, 'access', _1196 => _1196.body, 'optionalAccess', _1197 => _1197.statements, 'access', _1198 => _1198.forEach, 'call', _1199 => _1199((statement) => {
      if (_typescript2.default.isExpressionStatement(statement)) {
        const expression = statement.expression;
        if (_typescript2.default.isCallExpression(expression)) {
          const expressionText = expression.expression.getText().trim();
...
`;
  ...
  printVariableDeclarationList(node, identation) {
    const declaration = node.declarations[0];
    if (_optionalChain([declaration, 'optionalAccess', _1200 => _1200.name, 'access', _1201 => _1201.kind]) === _typescript2.default.SyntaxKind.ArrayBindingPattern) {
      const arrayBindingPattern = declaration.name;
      const arrayBindingPatternElements = arrayBindingPattern.elements;
      const parsedArrayBindingElements = arrayBindingPatternElements.map((e) => this.printNode(e.name, 0));
      const syntheticName = parsedArrayBindingElements.join("") + "Variable";
...
`;
  ...
  printThrowStatement(node, identation) {
    ...
    if (node.expression.kind === _typescript2.default.SyntaxKind.NewExpression) {
      const expression = node.expression;
      const argumentsExp = _nullishCoalesce(_optionalChain([expression, 'optionalAccess', _1202 => _1202.arguments]), () => ( []));
      const parsedArg = _nullishCoalesce(argumentsExp.map((n) => this.printNode(n, 0)).join(", "), () => ( ""));
      const newExpression = this.printNode(expression.expression, 0);
      if (expression.expression.kind === _typescript2.default.SyntaxKind.Identifier) {
        const id = expression.expression;
        const symbol = this.getChecker().getSymbolAtLocation(expression.expression);
        if (symbol) {
          const declarations = _nullishCoalesce(_optionalChain([this, 'access', _1203 => _1203.getChecker, 'call', _1204 => _1204(), 'access', _1205 => _1205.getDeclaredTypeOfSymbol, 'call', _1206 => _1206(symbol), 'access', _1207 => _1207.symbol, 'optionalAccess', _1208 => _1208.declarations]), () => ( []));
          const isClassDeclaration = declarations.find((l) => l.kind === _typescript2.default.SyntaxKind.InterfaceDeclaration || l.kind === _typescript2.default.SyntaxKind.ClassDeclaration);
          if (isClassDeclaration) {
...
function getProgramAndTypeCheckerFromMemory(rootDir, text, options = {}, cache) {
  ...
  const program = _typescript2.default.createProgram({
    ...
    host,
    oldProgram: _optionalChain([cache, 'optionalAccess', _1209 => _1209.memoryOldProgram])
  });
  if (cache !== void 0) {
