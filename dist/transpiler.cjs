...
`;
  ...
  csharpBoolCallTyped(node) {
    if (_optionalChain([node, 'optionalAccess', _130 => _130.kind]) !== _typescript2.default.SyntaxKind.CallExpression || !this.csharpIsCheckedBoolean(node)) {
      return false;
    }
    const expression = node.expression;
    if (_optionalChain([expression, 'optionalAccess', _131 => _131.kind]) !== _typescript2.default.SyntaxKind.PropertyAccessExpression || _optionalChain([expression, 'access', _132 => _132.expression, 'optionalAccess', _133 => _133.kind]) !== _typescript2.default.SyntaxKind.ThisKeyword) {
      return false;
    }
    let declaration;
    let checker;
    try {
      ...
      declaration = _optionalChain([checker, 'access', _134 => _134.getResolvedSignature, 'call', _135 => _135(node), 'optionalAccess', _136 => _136.declaration]);
    } catch (e) {
      return false;
    }
    ...
    return _optionalChain([checker, 'access', _137 => _137.getSymbolAtLocation, 'call', _138 => _138(declaration.name), 'optionalAccess', _139 => _139.declarations, 'optionalAccess', _140 => _140.length]) === 1;
  }
