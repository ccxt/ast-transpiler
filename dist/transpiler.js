...
`;
  ...
  csharpIsWriteTarget(node) {
    ...
    if (ts4.isBinaryExpression(parent) && parent.left === node) {
      const op = parent.operatorToken.kind;
      return op >= ts4.SyntaxKind.FirstAssignment && op <= ts4.SyntaxKind.LastAssignment;
    }
    if ((ts4.isPrefixUnaryExpression(parent) || ts4.isPostfixUnaryExpression(parent)) && parent.operand === node) {
