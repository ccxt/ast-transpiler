...
//  * handleErrors[4]: the hand-written BaseExchange.fetch() passes its
//    `Map<String, String> responseHeaders`, and `Map<String, Object>` is inconvertible.
//  * parseOrder[0]/parseTrade[0]: the venue overrides runtime-test the parameter for a
//    string (`typeof order === 'string'`, a string id is a valid input); a Map parameter
//    would make that branch dead and the call-site checkcast throw on it.
//  * the prediction-tier names: java/lib/.../exchanges/prediction/** is not regenerated by
//    the farm's java target (D-16 owns the tier), and its committed files call/declare
//    these with `Object` parameters, so a retyped base signature stops overriding there.
